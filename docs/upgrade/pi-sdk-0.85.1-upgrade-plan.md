...
依赖更新涉及 [Runtime Node](../../apps/runtime-node/package.json)、[Pi Server](../../packages/pi-runtime/pi-runtime-server/package.json)、[Pi Protocol](../../packages/pi-runtime/pi-rpc-contracts/package.json)、[Pi Shared](../../packages/pi-runtime/pi-runtime-adapters/package.json) 和 [Terminal Pi Tool](../../packages/pi-sdk/pi-sdk-terminal/package.json)。`chord` 等由 pnpm 解析为传递依赖，未额外声明。旧 `pi-client` / `pi-protocol` 传递依赖随新依赖闭包移除。
...
pnpm --filter @workbench/pi-rpc-contracts \
  --filter @workbench/pi-runtime-adapters \
  --filter @workbench/pi-runtime-server \
  --filter @workbench/pi-runtime-client \
  --filter @workbench/pi-sdk-terminal \
...
| 组别               | 文件（路径相对仓库根目录）                                                                                                                                                            | 结果                                       |
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| 消息契约           | `packages/pi-runtime/pi-runtime-adapters/tests/reducer.test.ts`；`packages/pi-runtime/pi-conversation-adapter/tests/session-message-accumulator.test.ts`                              | 8/8 通过                                   |
| 模型与请求         | `packages/pi-runtime/pi-runtime-server/test/models/{model-service,model-request-transport}.test.ts`                                                                                   | 与下面四组合计 128/128 通过                |
| 会话状态与恢复     | `packages/pi-runtime/pi-runtime-server/test/sessions/{session-queue,session-resume,session-interruption,session-context-policy,session-context-trace}.test.ts`                        | 通过                                       |
| 内置工具与技能     | `packages/pi-runtime/pi-runtime-server/test/internal-extensions/{builtin-tools,enhanced-search}.test.ts`；`packages/pi-runtime/pi-runtime-server/tests/skills/builtin-skills.test.ts` | 通过                                       |
| 终端执行           | `packages/pi-sdk/pi-sdk-terminal/tests/interactive-bash-tool.test.ts`                                                                                                                 | 通过                                       |
...
| 模型默认与压缩边界 | `packages/pi-runtime/pi-runtime-server/tests/sessions/session-registry-metadata.test.ts`，以及同目录的 context-policy、resume、queue、context-trace 测试                              | 74/74 通过；其中四个文件与上方已有回归重复 |
