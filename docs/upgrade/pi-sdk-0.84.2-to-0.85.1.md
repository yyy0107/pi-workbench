...
| `@workbench/agent-runtime-pi-server`   | `pi-ai: 0.84.2`、`pi-coding-agent: 0.84.2` | [server/package.json](../../packages/pi/server/package.json)                  |
| `@workbench/agent-runtime-pi-protocol` | `pi-ai: 0.84.2`                            | [protocol/package.json](../../packages/pi/protocol/package.json)              |
| `@workbench/agent-runtime-pi-shared`   | `pi-ai: 0.84.2`                            | [shared/package.json](../../packages/pi/shared/package.json)                  |
...
这里的 `mode: "rpc"` 是嵌入会话的扩展绑定模式；Workbench 的网络接口仍由自身 Host 提供。本次没有发现应用代码直接导入 `pi-coding-agent/client`；该字符串出现在 Runtime 构建脚本的解析分支中。[运行时边界说明](../../packages/pi/README.md)、[服务创建](../../packages/pi/model-server/src/agent-session-services.ts)、[会话创建与绑定](../../packages/pi/session-server/src/session-registry.ts)
...
基线补丁 `patches/@earendil-works__pi-coding-agent@0.84.2.patch` 同时修改 `dist/index.js` 和 `dist/index.d.ts`（升级后由 [0.85.1 等效补丁](../../patches/@earendil-works__pi-coding-agent@0.85.1.patch)替换）。Workbench 通过 [public/installation.ts](../../packages/pi/server/src/public/installation.ts) 将其提供给 [installed-api-only-runtime-host.ts](../../apps/runtime-node/src/installed-api-only-runtime-host.ts)，用于控制 stdout 的接管与恢复。
...
| UI prompt 事件           | `ui_prompt_start` / `ui_prompt_end`                                                 | Workbench 已由 `InteractiveResponseRegistry` 提供等待用户状态。若采用新事件，应通过现有状态所有者适配，避免重复计时或维护两套等待状态。[现有交互注册表](../../packages/pi/session-server/src/interactive-response-registry.ts)                                                                                                 |
...
| PowerShell               | `createPowerShellTool()` / `createPowerShellToolDefinition()` 等                    | Workbench 有自己的终端 bash override、工具偏好和来源标记；本次升级不自动增加独立 PowerShell 产品能力。[当前工具组合](../../packages/pi/tools/src/builtin-tools.ts)                                                                                                                                                             |

## 6. 现有行为的重点回归范围

### 6.1 消息流、持久化与恢复

当前 [protocol/src/stream.ts](../../packages/pi/protocol/src/stream.ts) 使用 `PiMessagesEvent` 的内容事件，并排除 `start`、`done`、`error`；服务器将它们写成 `format: "pi-messages-v1"` 的持久化 chunk，客户端由共享 reducer 和 accumulator 重建。
...
下一步应以交错 text/thinking/tool-call、部分 JSON、请求开始前失败、签名和最终消息元数据、重连与历史重放为代表场景。现有 `copyPiAssistantMessage()` 和 `assistantMessageMetadata()` 会复制额外字段；需验证端到端是否保留新字段，而不是先认定必须修改所有 DTO。[共享 reducer](../../packages/pi/shared/src/messages/reducer.ts)、[客户端 accumulator](../../packages/pi/conversation/src/session-message-accumulator.ts)
...
Workbench 的 [session-registry.ts](../../packages/pi/session-server/src/session-registry.ts) 同时拥有 prompt、队列、取消、上下文策略及会话状态投影；[session-context-trace.ts](../../packages/pi/session-server/src/session-context-trace.ts) 消费 compaction/retry 生命周期。因此即使升级后类型检查通过，也需要针对事件顺序和运行结束状态做回归。
...
请求检查优先使用结构化 fake 或已安装的测试能力，不依赖真实付费模型请求。当前宿主 fetch 隔离逻辑需要保留，并检查新增 Deferred 入口是否进入实际调用范围。[模型请求适配](../../packages/pi/model-server/src/agent-session-services.ts)
...
文档同步范围包括 [内置 pi-docs](../../packages/pi/server/resources/skills/pi-docs/SKILL.md) 及其 `references/`（当前明确标记 0.84.2），以及 [Pi AI 技能](../../.agents/skills/pi-ai-sdk/SKILL.md)、[Coding Agent source-routing](../../.agents/skills/pi-coding-agent-sdk/references/source-routing.md) 中的入口说明。后者当前将 `./client` 列为可用包入口，需在升级时按新发布条件修正。
...
| 流重建                 | [reducer.test.ts](../../packages/pi/shared/tests/reducer.test.ts)、[session-message-accumulator.test.ts](../../packages/pi/conversation/tests/session-message-accumulator.test.ts)                                                                                                                                                                                                                                                                                                                    |
| 会话与上下文           | [session-queue.test.ts](../../packages/pi/session-server/tests/session-queue.test.ts)、[session-interruption.test.ts](../../packages/pi/session-server/tests/session-interruption.test.ts)、[session-resume.test.ts](../../packages/pi/server/tests/sessions/session-resume.test.ts)、[session-context-policy.test.ts](../../packages/pi/session-server/tests/session-context-policy.test.ts)、[session-context-trace.test.ts](../../packages/pi/server/tests/sessions/session-context-trace.test.ts) |
| 模型与请求隔离         | [model-service.test.ts](../../packages/pi/server/tests/models/model-service.test.ts)、[model-request-transport.test.ts](../../packages/pi/server/tests/models/model-request-transport.test.ts)                                                                                                                                                                                                                                                                                                        |
| 内置工具与终端         | [builtin-tools.test.ts](../../packages/pi/server/tests/internal-extensions/builtin-tools.test.ts)、[interactive-bash-tool.test.ts](../../packages/terminal/pi-tool/tests/interactive-bash-tool.test.ts)                                                                                                                                                                                                                                                                                               |
...
node --no-warnings=ExperimentalWarning \
  ...
  --test packages/pi/shared/tests/reducer.test.ts
```
