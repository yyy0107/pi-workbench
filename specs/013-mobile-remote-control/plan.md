...
**Structure Decision**: Keep React Native product code app-local because only the phone consumes it and existing client UI is DOM/Electron-specific. Extract only protocol, state-machine transport, Relay domain, and Pi desktop bridge capabilities. Compose the bridge in Electron main—where the local Runtime connection and secure credential lifecycle already live—rather than changing `runtime-node` or constructing a second Pi service. Compose the Relay in its own deployable app so the server library stays storage/listener/provider independent.
...
| `apps/mobile`                 | Expo product, native adapters, routes/screens/i18n                                                      | Remote client/contracts, shared i18n runtime, Expo/native libraries                                          | Desktop UI/Shell, agent Runtime client, Pi Runtime/client, Electron, extensions/toolbox |
