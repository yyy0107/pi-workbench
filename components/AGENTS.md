...
- `components/ui`、`components/elements` 和图标等通用组件保持 locale 无关：不导入业务词典，不读取业务命名空间。
...
- `components/assistant-ui` 的界面文案统一归入 `assistant` 命名空间，并集中在 `components/assistant-ui/i18n/<locale>.ts`；通用 `components/ui` 不读取该词典。
