  ...
  return (
    <div data-slot="aui_thread-list-search" className="relative px-0.5 py-1">
      ...
      <Input
        ...
        aria-label={t("assistant.threads.search")}
  ...
  const { filteredIndices, groups } = useMemo(() => {
    ...
    const filteredIndices = threadIds
      ...
      .filter(
        ({ id }) =>
          !query ||
          (itemsById.get(id)?.title || t("assistant.threads.newChat"))
            .toLowerCase()
            .includes(query),
      )
      .map(({ index }) => index);
    ...
    for (const index of sorted) {
      const label = dateGroupLabel(dates[index], startOfToday, {
        today: t("assistant.threads.today"),
        yesterday: t("assistant.threads.yesterday"),
        earlier: t("assistant.threads.earlier"),
      });
    ...
    <div className="flex flex-col gap-0.5">
      {Array.from({ length: 5 }, (_, i) => (
        <div
          key={i}
          role="status"
          aria-label={t("assistant.threads.loading")}
      ...
      {isRenaming ? (
        ...
        >
          ...
          <span data-slot="aui_thread-list-item-title" className="min-w-0 flex-1 truncate">
            <ThreadListItemPrimitive.Title fallback={t("assistant.threads.newChat")} />
          </span>
          {isRunning && <span className="sr-only">{t("assistant.threads.running")}</span>}
        </ThreadListItemPrimitive.Trigger>
      )}
  ...
  return (
    <ThreadListItemMorePrimitive.Root sharedFocusGroup>
      ...
      >
        ...
        <span className="sr-only">{t("assistant.threads.moreOptions")}</span>
      </ThreadListItemMorePrimitive.Trigger>
      ...
      >
        ...
        >
          ...
          {t("assistant.threads.renameAction")}
        </ThreadListItemMorePrimitive.Item>
