...
import { useWorkspaceFileRuntime } from "@workbench/workspace-files";

import { fileBreadcrumbSegments, fileBreadcrumbTreeRootPath } from "../lib/file-breadcrumb-model";
...
export function FileBreadcrumbTree({ surface, context }: WorkspaceSurfaceProps<FileSurfaceParams>) {
  ...
  const treeLabels = {
    tree: treeT("workspaceFiles.fileTree.tree"),
    empty: treeT("workspaceFiles.fileTree.empty"),
    noMatches: treeT("workspaceFiles.fileTree.noMatches"),
    loadingDirectory: ({ name }: { name: string }) =>
      treeT("workspaceFiles.fileTree.loadingDirectory", { name }),
    loadDirectoryError: ({ name }: { name: string }) =>
      treeT("workspaceFiles.fileTree.loadDirectoryError", { name }),
    retryDirectory: ({ name }: { name: string }) =>
      treeT("workspaceFiles.fileTree.retryDirectory", { name }),
    emptyDirectory: ({ name }: { name: string }) =>
      treeT("workspaceFiles.fileTree.emptyDirectory", { name }),
  };
            ...
            >
              ...
              >
                <div className="min-h-0 flex-1 p-1.5">
                  ...
                  (rootState.status === "loading" && rootState.nodes.length === 0) ? (
                    ...
                    >
                      ...
                      {treeT("workspaceFiles.fileTree.loading")}
                    </div>
                    ...
                    >
                      ...
                      <p>{treeT("workspaceFiles.fileTree.loadError")}</p>
                      <Button
                        type="button"
                        variant="outline"
                        size="sm"
                      ...
                      >
                        {treeT("workspaceFiles.fileTree.retry")}
                      </Button>
                    </div>
                    ...
                    <ExplorerTree
                      ...
                      onOpenFileError={(_error, node) =>
                        notifications.add({
                          ...
                          type: "error",
                          priority: "high",
                          title: treeT("workspaceFiles.fileTree.openError", { name: node.name }),
                        })
                      }
                    />
                  ) : null}
                ...
                {truncated ? (
                  ...
                  >
                    {treeT("workspaceFiles.fileTree.truncated")}
                  </div>
                ) : null}
              </PopoverContent>
            </Popover>
