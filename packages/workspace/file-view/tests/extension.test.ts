...
const { workspaceFileExtension } = (await import(
  new URL("../src/extension.ts", import.meta.url).href
)) as typeof import("../src/extension");
const { createWorkspaceFileOpenersBinding, registerWorkspaceFileOpeners } = (await import(
  new URL("../src/file-openers-bridge.tsx", import.meta.url).href
)) as typeof import("../src/file-openers-bridge");

const openerIds = ["workspace.file"];
...
test("Workspace File opener registration preserves the existing handler after a collision", () => {
  const manager = new ExtensionManager();
