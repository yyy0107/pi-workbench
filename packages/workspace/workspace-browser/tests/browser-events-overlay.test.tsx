...
import { Button, Dialog, Input, ToastProvider } from "@workbench/ui";
import {
  BROWSER_SESSION_SERVICE_RESOURCE,
  MemoryBrowserSessionService,
} from "../src/browser-session-service";
import { BrowserEventsOverlay, resolveEmbeddedBrowserUrl } from "../src/browser-events-overlay";
import { BrowserDownloadsDialog } from "../src/browser-downloads-dialog";
...
test("browser overlay waits for explicit responses, routes confirmed links, and manages downloads", async () => {
  ...
  let surfaceCount = 0;
  let workspace!: ReturnType<typeof useRightWorkspaceStateStore>;
  ...
  function Probe() {
    workspace = useRightWorkspaceStateStore();
  ...
  try {
    ...
    const opener = Object.values(workspace.getState().surfaces)[0]!;
    ...
    const currentWorkspace = workspace.getState();
