...
拥有文件服务/缓冲、文件树与图标、文件链接/菜单、本地应用选择与偏好、下载以及文件分类。根入口提供服务，`/tree`、`/links`、`/classification`、`/open-apps`、`/open-preferences`、`/app-icon`、`/download`、`/markdown-links`、`/i18n` 提供明确的能力入口。
...
内部辅助：`lib/asset-module-url.ts`, `lib/file-classification.ts`, `lib/file-link-content.ts`, `lib/tree/file-name-parts.ts`, `lib/tree/material-icon-theme.ts`.
