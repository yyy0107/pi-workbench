 ...
 *
 * Individual viewers always pass their injected asset base explicitly. Some third-party preset
...
export function acquireFileViewerAssetBaseLease(baseUrl: string): () => void {
  ...
  if (normalizedBaseUrl === undefined) {
    throw new Error("A File Viewer asset base URL is required for a Workbench installation.");
  }
  ...
  if (activeLeaseCount === 0) {
    ...
    throw new Error(
      `Cannot install File Viewer assets for ${normalizedBaseUrl}: ${activeBaseUrl} is already active.`,
    );
  }
