...
import { saveBrowserFile } from "../lib/browser-files";
  ...
  if (!session)
    return (
      ...
      >
        <span>
          {workspaceT(
            error ? "rightWorkspace.status.disconnected" : "rightWorkspace.status.loading",
          )}
        </span>
        {error ? (
        ...
        >
          <span className="flex-1">
            {session.status === "disconnected"
              ? workspaceT("rightWorkspace.status.disconnected")
              : t("extensions.workspaceBrowser.settings.actionError")}
          </span>
