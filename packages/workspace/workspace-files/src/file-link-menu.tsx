...
import { fileLinkResource, openFileLink } from "./file-link";
...
function FileLinkMenuItems({ href }: { href: string }) {
  ...
  const primaryLabel = primary
    ? t("workspaceFiles.openWith", { name: primary.name })
    : t("workspaceFiles.openFile");
  ...
  const run = async (action: () => Promise<unknown>, success?: string) => {
    ...
    try {
      ...
      notifications.add({
        type: "error",
        title: t("workspaceFiles.linkMenu.actionFailed"),
      });
    } finally {
      setPending(false);
    }
  ...
  const openWith = async (app?: WorkbenchLocalApp) => {
    ...
    if (
      ...
      notifications.add({ type: "error", title: t("workspaceFiles.saveBeforeBrowser") });
      return;
    }
    ...
    try {
      ...
      notifications.add({
        type: "error",
        title: t("workspaceFiles.openPreferenceSaveFailed"),
      });
    }
  };
  ...
  const status =
    descriptor === null
      ? t("workspaceFiles.loadFailed")
      : apps === null
        ? t("workspaceFiles.localAppsLoadError")
        : preferenceError
          ? t("workspaceFiles.openPreferenceLoadFailed")
          : !descriptor
            ? t("workspaceFiles.loading")
            : !apps
              ? t("workspaceFiles.loadingLocalApps")
              : undefined;
  ...
  return (
    <>
      <ContextMenuItem
        ...
        disabled={pending}
      >
        {t("workspaceFiles.linkMenu.openFile")}
      </ContextMenuItem>
      <ContextMenuItem
        ...
        title={
          (primary?.kind === "browser" || (!primary && fileKind === "html")) && dirty
            ? t("workspaceFiles.saveBeforeBrowser")
            : undefined
        }
      >
        ...
        <ContextMenuSubContent>
          {choices.map((app) => (
            <ContextMenuItem
              ...
              title={
                app.kind === "browser" && dirty ? t("workspaceFiles.saveBeforeBrowser") : undefined
              }
            >
          ...
          <ContextMenuItem
            ...
            title={fileKind === "html" && dirty ? t("workspaceFiles.saveBeforeBrowser") : undefined}
          >
            {t("workspaceFiles.openFile")}
          </ContextMenuItem>
        </ContextMenuSubContent>
      ...
      >
        {t("workspaceFiles.linkMenu.saveAs")}
      </ContextMenuItem>
      <ContextMenuItem
        data-inset
        disabled={!path || path.startsWith("~") || pending}
        onClick={() => run(() => copy(path!), t("workspaceFiles.linkMenu.pathCopied"))}
      >
        {t("workspaceFiles.linkMenu.copyPath")}
      </ContextMenuItem>
      <ContextMenuItem
        data-inset
        ...
        onClick={() =>
          run(
            ...
            t("workspaceFiles.contentCopied"),
          )
        }
      >
        {t("workspaceFiles.copyContent")}
      </ContextMenuItem>
      <ContextMenuItem
        data-inset
      ...
      >
        {t("workspaceFiles.linkMenu.showInFolder")}
      </ContextMenuItem>
      {status ? (
        <>
          <ContextMenuSeparator />
