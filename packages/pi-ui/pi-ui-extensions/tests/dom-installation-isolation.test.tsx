...
test("production DOM IDREF literals use the exact domain allowlist", async () => {
  ...
  const piSourceRoots = [
    ...
    .flatMap((capability) =>
      ["src", "lib"].map((root) =>
        fileURLToPath(new URL(`../../${capability}/${root}/`, import.meta.url)),
      ),
    )
    .filter(existsSync);
  for (const filename of (await Promise.all(piSourceRoots.map(sourceFiles))).flat()) {
  ...
  const runningIndicator = await readFile(
    new URL("../../pi-ui-status/src/pi-running-indicator.tsx", import.meta.url),
    "utf8",
  );
...
test("Pi consumers read the shared finite DOM seam through the public context entry", async () => {
  const sources = await Promise.all(
    [
      "../../pi-ui-diagnostics/src/context-trace-trigger.tsx",
      "../../../workspace/workspace-file-view/src/file-surface-header.tsx",
    ].map((relative) => readFile(new URL(relative, import.meta.url), "utf8")),
  );

  for (const source of sources) {
    assert.match(source, /useWorkbenchDomIds/u);
    assert.match(source, /from "@workbench\/shell-context\/dom"/u);
  }
