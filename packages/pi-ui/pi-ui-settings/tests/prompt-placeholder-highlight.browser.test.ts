  ...
  () => {
    ...
    try {
      const source = stripTypeScriptTypes(
        readFileSync(new URL("../lib/prompt-placeholder-highlight.ts", import.meta.url), "utf8"),
      );
      const css = readFileSync(
        new URL("../src/prompt-placeholder-highlight.module.css", import.meta.url),
        "utf8",
      );
      ...
      try {
        ...
        await tick();
        check(JSON.stringify(texts()) === JSON.stringify(['{{pi.tool_guidelines}}', '{{pi.cwd}}', '{{pi.terminal_environment}}']), 'Preview replacement removes old ranges and preserves all editor tokens');
        clearEditor();
