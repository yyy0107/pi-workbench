...
import { supportedModelThinkingLevels } from "@workbench/pi-runtime-adapters/models";
import { piSettingsModelsTranslationBundle as piTranslationBundle } from "../src/i18n";
import { uiTranslationBundle } from "@workbench/ui/i18n";
import { emptyModel, toModelDraft, type ModelDraft } from "../lib/model-config-draft";
import {
  ModelCatalogRow,
  setReasoningLevelSupported,
  type ModelCatalogRowTestResult,
} from "../src/model-config-model-row";
  ...
  return renderToStaticMarkup(
    <WorkbenchSettingsProvider service={{ load: async () => ({}), update: async () => undefined }}>
      <I18nProvider
        locale={locale}
        onLocaleChange={() => {}}
        bundles={[piTranslationBundle, uiTranslationBundle]}
      >
        <ModelCatalogRow
          model={model}
          index={0}
