...
import {
  fileMutationResourceStats,
  fileMutationToolModel as toolDiffModel,
} from "../lib/file-mutation-tool-model";
...
test("keeps empty diffs renderable as fallback and reports only completed basename stats", () => {
  const empty = toolDiffModel({
    toolName: "write",
    toolCallId: "write-empty",
    args: { path: "/one/shared.ts", content: "" },
  });
  assert.equal(empty?.filename, "shared.ts");
  assert.deepEqual(empty?.hunks, []);
  const block = {
    key: "write",
    kind: "tool-call",
    callId: "write-complete",
    toolName: "write",
    arguments: { path: "/two/shared.ts", content: "一\ntwo\n" },
    argumentsText: "{}",
    status: "complete",
  } as const;
  assert.deepEqual(fileMutationResourceStats(block), [{ file: "shared.ts", added: 2 }]);
  assert.deepEqual(fileMutationResourceStats({ ...block, status: "error" }), []);
});
