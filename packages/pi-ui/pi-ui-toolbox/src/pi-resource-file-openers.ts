...
import type { PiResourceClient } from "@workbench/pi-runtime-client/resources";
import type {
...
import { fileWorkspaceContext, type FileWorkspaceService } from "@workbench/workspace-files";
...
export function createPiResourceFileOpenHandlers(
  files: FileWorkspaceService,
  resources: Pick<
    PiResourceClient,
    "listExtensionFiles" | "listSkillFiles" | "readExtensionFile" | "readSkillFile"
  >,
): Readonly<{
  skillFileOpenHandler: OpenHandlerDefinition;
  skillDirectoryOpenHandler: OpenHandlerDefinition;
  extensionFileOpenHandler: OpenHandlerDefinition;
  extensionDirectoryOpenHandler: OpenHandlerDefinition;
}> {
  const skillFileOpenHandler = {
    ...
    open: async ({ resource, context, scope, policy }, { surfaces }) => {
      ...
      const skillFile = await client.readSkillFile({ ...request, name: skillName, relativePath });
      const fileSession = {
      ...
      const snapshot = files.attachFile(
  ...
  const skillDirectoryOpenHandler = {
    ...
    open: async ({ resource, context, scope, policy }, { surfaces }) => {
      ...
      const client = resources;
      const listing = await client.listSkillFiles({ ...request, name: skillName });
      const targetScope =
        scope ??
        (context.threadId
          ? { type: "thread" as const, key: context.threadId }
  ...
  const extensionFileOpenHandler = {
    ...
    open: async ({ resource, context, scope, policy }, { surfaces }) => {
      ...
      const targetScope =
        ...
        (context.threadId
          ? { type: "thread" as const, key: context.threadId }
          : { type: "application" as const, key: context.applicationId });
      const extensionFile = await client.readExtensionFile(identity);
      const fileSession = {
  ...
  const extensionDirectoryOpenHandler = {
    id: "workspace.directory.extension",
    canOpen: ({ resource }) => {
      const identity = extensionFileIdentity(resource.metadata);
      return resource.scheme === "extension-directory" && resource.path.trim() && identity
        ? 100
        : 0;
    },
    open: async ({ resource, context, scope, policy }, { surfaces }) => {
      ...
      const client = resources;
      const listing = await client.listExtensionFiles(identity);
      const targetScope =
        scope ??
        (context.threadId
          ? { type: "thread" as const, key: context.threadId }
