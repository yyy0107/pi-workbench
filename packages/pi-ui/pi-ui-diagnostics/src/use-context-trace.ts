...
import { selectContextTraceActivation } from "../lib/context-trace-activation";
...
export function useContextTrace(
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      const value = await traceClient.list({
        sessionId,
  ...
  }, [requestedActivationId, sessionId, snapshot, traceClient]);
