...
export function decorateMarkdownLinks(isLocalFileHref: (href: string) => boolean) {
  return (tree: MarkdownNode) =>
    visitLinks(tree, (node) => {
      if (node.type === "element" && node.tagName === "a") {
        const href = node.properties.href;
        if (typeof href !== "string" || !href) return;
        if (isLocalFileHref(href)) node.tagName = "workbench-file-link";
...
export function MarkdownFileLink({
  ...
  const { FileLink } = useMarkdownLinkAdapter();
  return href ? (
