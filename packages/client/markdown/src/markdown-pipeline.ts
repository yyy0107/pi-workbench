...
export function markdownToHast(
  ...
  idPrefix: string,
  isLocalFileHref: (href: string) => boolean = () => false,
): HastRoot {
  ...
  visitElements(tree, (element) => {
    ...
    else if (isLocalFileHref(href) && /^[a-z]:[\\/]/i.test(href))
      properties.href = `file:///${href.replaceAll("\\", "/")}`;
    else if (!/^file:/i.test(href) && /^[^/]+:/.test(href) && isLocalFileHref(href))
