...
test("composer presentation leaves do not read runtime or workbench state", async () => {
  const source = await readFile(new URL("src/workbench-composer-view.tsx", packageRoot), "utf8");
...
test("composer container owns runtime integration and supplies typed views", async () => {
  const source = await readFile(new URL("src/workbench-composer.tsx", packageRoot), "utf8");
