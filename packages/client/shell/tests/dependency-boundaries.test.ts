...
test("shared UI foundations do not depend on chat, elements, or a conversation runtime", () => {
  assertNoImports(productionFilesUnder(path.resolve(PACKAGE_ROOT, "../ui/src")), (source, file) => {
    if (
...
test("RightWorkspace core remains independent from conversation and Agent runtimes", () => {
  const boundaryAdapters = new Set([
    "right-workspace-persistence.ts",
...
test("AI presentation leaves do not read session, transport, or external store state", () => {
  const elementFiles = productionFilesUnder(
    path.resolve(PACKAGE_ROOT, "../../conversation/conversation/src/elements"),
  );
...
test("fallback and built-in message renderers share the Workbench Block host and leaves", () => {
  const conversationSource = path.resolve(PACKAGE_ROOT, "../../conversation/conversation/src");
  const owners = [
    path.join(conversationSource, "message-parts.tsx"),
    path.join(conversationSource, "message-presentation/message-presentation.tsx"),
  ];
