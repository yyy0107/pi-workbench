...
import type { TranslationBundle } from "@workbench/i18n/runtime";
...
test("localizes recorded compaction reasons without treating every compaction as a threshold", () => {
  ...
  for (const [reason, en, zh] of [
    ...
    assert.equal(
      english
        .forBundle(conversationTranslationBundle)
        .t("workbench.chat.separators.contextCompactionReason", { reason }),
