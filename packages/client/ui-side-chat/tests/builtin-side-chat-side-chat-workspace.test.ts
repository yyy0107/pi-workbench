...
import { uiSideChatTranslationBundle, defineMessage } from "../src/i18n";
const createI18n = (locale: Locale) => createRuntime(locale, [uiSideChatTranslationBundle]);
...
test("renders the sequence after the localized side-chat title", () => {
  ...
  assert.equal(createI18n("en-US").text(title), "Temporary chat (2)");
  assert.equal(createI18n("zh-CN").text(title), "临时侧聊 (2)");
});

test("keeps legacy unnumbered side-chat title descriptors resolvable", () => {
  const title = defineMessage("extensions.sideChat.title");

  assert.equal(createI18n("en-US").text(title), "Temporary chat");
  assert.equal(createI18n("zh-CN").text(title), "临时侧聊");
});
