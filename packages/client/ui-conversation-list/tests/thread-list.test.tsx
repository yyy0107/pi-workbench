...
import { NewThreadWorkspaceItem } from "@workbench/workspace-directory-picker";
import { useNewThreadLayout } from "@workbench/shell-context/layout";
import { WorkspaceSidebarProvider, useWorkspaceSidebar } from "../src/sidebar-context";
...
test("workspace conversations isolate selection updates and paginate independently on collapse", async () => {
  ...
  const RowProbe = memo(function RowProbe({ id }: { id: string }) {
    activeRows.set(
      id,
      useWorkspaceSidebar((state) => state.activeThreadId === id),
    );
    routedRows.set(
      id,
  ...
  function Probe(props: ComponentProps<typeof WorkbenchThreadList>) {
    const sidebar = useWorkspaceSidebar((state) => ({ model: state.model }));
    model = sidebar.model;
        ...
        <RuntimeProvider runtime={runtime}>
          <WorkbenchSettingsProvider service={settings}>
            <I18nProvider
              ...
              bundles={[sidebarTranslationBundle, sharedSidebarTranslationBundle]}
            >
              <WorkspaceSelectionProvider
...
import { useWorkspaceSidebarItem } from "../src/workspace-sidebar-item";
import { sidebarThreadKey } from "../src/sidebar-move";
