...
import {
  ...
  SidebarRow,
  SidebarStatus,
} from "@workbench/ui-sidebar/primitives";
...
import { SidebarMoveMenuItems } from "./sidebar-move-menu-items";
import { sidebarWorkspaceKey } from "./sidebar-move";
...
export function WorkbenchPinnedThreadList({ onNavigate }: { onNavigate?: () => void }) {
  const sidebar = useWorkspaceSidebar(
    useShallow((state) => ({
      ...
      revealedWorkspaceId: state.revealedWorkspaceId,
...
export function WorkbenchWorkspaceThreadList({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const {
    directories,
    selection,
    workspaceActions: { destroyNewThread },
  } = sidebar;
...
const WorkspaceDirectorySection = memo(function WorkspaceDirectorySection({
  ...
  const remove = async () => {
    try {
      await sidebar.workspaceActions.removeWorkspace(directory.id);
      if (active) {
        sidebar.switchToNewThread();
        sidebar.openHome();
        onNavigate?.();
      }
    } catch (error) {
  ...
  return (
    <SidebarGroup
      ...
      onOpenChange={(open) => {
        ...
        sidebar.workspaceActions.activateWorkspace(directory.id);
        if (open !== expanded) sidebar.workspaceActions.setWorkspaceCollapsed(directory.id, !open);
      }}
      header={
        <SidebarRow
          variant="folder"
