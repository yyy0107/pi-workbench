...
} from "@workbench/ui-message-blocks/conversation-separator";
import { DisclosureScrollDirectionProvider } from "@workbench/ui-disclosure";
import { ErrorState } from "@workbench/ui-message-blocks/error-state";
...
import { WorkbenchComposerCommandResponse } from "@workbench/ui-message-blocks/composer-command-response";
...
import { isLastAssistantInTurn } from "../lib/message-action-visibility";
import { isMessageInLatestTurn, shouldShowMessageError } from "../lib/workbench-message-error";
...
function WorkbenchMessageError() {
  ...
  const { t: blocksT } = useI18n(blocksTranslationBundle);
  ...
  let title = blocksT("workbench.chat.errors.requestFailedTitle");
  let detail = rawDetail ?? blocksT("workbench.chat.errors.unknownFailure");
  ...
  return (
    <ErrorState
      ...
      retryLabel={
        canContinue ? t("workbench.chat.errors.continue") : blocksT("workbench.chat.errors.retry")
      }
      retryingLabel={
        canContinue
          ? t("workbench.chat.errors.continuing")
          : blocksT("workbench.chat.errors.retrying")
      }
      onRetry={retry}
...
export function WorkbenchSystemMessage() {
  ...
  const compactionReason =
    conversationEvent?.kind === "compaction"
      ? blocksT("workbench.chat.separators.contextCompactionReason", {
