"use client";
import { uiSettingsArchivedChatsTranslationBundle } from "./i18n";
...
import { archivedChatGroupHeadingId } from "../lib/archived-chat-group-a11y";
...
export function ArchivedChatsSettingsItem({ sectionId, itemId }: SettingsItemComponentProps) {
  const { date: formatDate, locale, t } = useI18n(uiSettingsArchivedChatsTranslationBundle);
      ...
      {isLoading && archivedChats.length === 0 ? (
        ...
        <div className="space-y-6">
          {visibleGroups.map((group) => {
            const headingId = archivedChatGroupHeadingId(archivedChatGroupIdPrefix, group.id);
            return (
