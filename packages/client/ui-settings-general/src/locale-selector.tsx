...
import { SettingsRow } from "@workbench/ui";
import { buttonVariants } from "@workbench/ui";
import {
  DropdownMenu,
  DropdownMenuContent,
...
import { createLocaleDisplayName } from "../lib/locale-display-name";

function useSelectLocale() {
  const navigation = useWorkbenchNavigation();
  const { setLocale } = useI18n(settingsGeneralTranslationBundle);
...
function LocaleSelector({ compact = false }: { compact?: boolean }) {
  const { locale, t } = useI18n(settingsGeneralTranslationBundle);
