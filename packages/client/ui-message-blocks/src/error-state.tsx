...
import { ShimmerLabel } from "@workbench/ui";
...
export function ErrorState({
  ...
  if (retrying) {
    return (
      <div
        ...
        {...props}
      >
        <RefreshCwIcon className="text-foreground/45 size-(--icon-size-sm) shrink-0 [--button-icon-size:var(--icon-size-sm)] animate-spin motion-reduce:animate-none" />
  ...
  return (
    ...
    >
      {tone === "error" ? (
        <CircleAlertIcon className="mt-0.5 size-(--icon-size-sm) shrink-0 [--button-icon-size:var(--icon-size-sm)] text-red-500/80" />
      ) : (
        <CircleStopIcon className="text-muted-foreground mt-0.5 size-(--icon-size-sm) shrink-0 [--button-icon-size:var(--icon-size-sm)]" />
      )}
