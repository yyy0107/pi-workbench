...
import { ReasoningPanel, type ReasoningStep } from "./reasoning-panel";
import { ToolCall, ToolCallDetails } from "./tool-call";
import { ShimmerLabel, withTooltip } from "@workbench/ui";
...
function TimelineToolCall({
  ...
  const { locale, t, text } = useI18n(conversationTranslationBundle);
  ...
  const displayActiveLabel = activeLabel;
  ...
  const terminalLabel =
    presentation?.terminalLabel !== undefined
      ...
      : state.cancelled
        ...
        : failedLabel;
  const SummaryComponent = presentation?.summaryComponent;
  const displayedSummaryLabel =
  ...
  const ordinarySummary = (
    <span className="flex min-w-0 items-center">
      ...
      >
        {displayedSummaryLabel}
      </ShimmerLabel>
      {withTooltip(
        <span title={query} className="ms-1 min-w-0 truncate font-mono text-foreground/70">
          {query}
        </span>,
      )}
    </span>
  );
  const customSummary: ReactNode = SummaryComponent ? (
    <ExtensionErrorBoundary
      contributionId={`tool-summary:${block.toolName}`}
      source="renderer"
      resetKey={block}
      fallback={ordinarySummary}
    >
      <SummaryComponent node={node} block={block} label={displayedSummaryLabel} />
    </ExtensionErrorBoundary>
  ) : undefined;
  let expandable = true;
  try {
    expandable = presentation?.getExpandable?.(block, node) ?? true;
  } catch {
    expandable = true;
  }
  ...
  return (
    <ToolCall
      ...
      showCompletionIcon={presentation?.showCompletionIcon ?? !presentation?.compact}
...
export function MessageToolTimeline({
  ...
  const entries = useMemo(
    () =>
      timelineEntries(
        ...
        toolPresentations,
        node,
        stepModels.map((model) => (model.kind === "data" ? undefined : model.presentation)),
      ),
    [timelineBlocks, groupParallelTools, groups, toolPresentations, node, stepModels],
  );
  const stats = useMemo(
    () =>
      timelineStats(
    ...
    [timelineBlocks, toolPresentations, node, stepModels],
  );
  ...
  const latestActivity = (() => {
    ...
    const label = presentationActiveLabel
      ? text(presentationActiveLabel)
