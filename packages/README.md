...
名称规则与旧名迁移见 [package-naming.md](../docs/package-naming.md) 、[Pi 名称映射](../docs/package-layout-map.json) 和 [基础设施映射](../specs/009-host-infrastructure-naming/plan.md)。已有协议、扩展、工具、持久化和翻译 bundle ID 保持稳定。
...
| [build/artifact-policy](build/artifact-policy)                                 | Workbench 构建与启动采用的产物准入规则：源码形态、Runtime 原生依赖、模型可读取资源和 Next standalone 依赖补齐规则。                                |
| [build/artifact-reader](build/artifact-reader)                                 | 在 Node 中读取并验证 Runtime / Web 构建产物。                                                                                                      |
...
| [contracts/runtime-contracts](contracts/runtime-contracts)                     | 共享 Runtime 连接、宿主能力、进程控制和构建产物格式合同。                                                                                          |
...
| [process/application-process](process/application-process)                     | Workbench Runtime 与 Web 服务进程的生命周期能力。                                                                                                  |
| [product/pi-workbench-runtime](product/pi-workbench-runtime)                   | Workbench 自定义工具、产品规则、默认扩展、Skills/Prompts 与部署策略。                                                                              |
...
| [transport/runtime-transport-client](transport/runtime-transport-client)       | Runtime HTTP 与 WebSocket 的客户端传输载体。                                                                                                       |
| [transport/runtime-transport-server](transport/runtime-transport-server)       | Node 端 Runtime HTTP / WebSocket 传输入口：提供 HTTP server、Fetch/Node 请求适配、连接认证及到 Runtime 子进程的代理。                              |
