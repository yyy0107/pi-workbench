...
const { ModelConfigStore } = (await import(
  new URL("../src/model-config-store.ts", import.meta.url).href
)) as typeof import("../src/model-config-store");
...
test("saved and migrated built-in models retain reasoning replay through the Pi SDK", async (t) => {
  ...
  const { createWorkbenchAgentSessionServices } = await import("../src/agent-session-services");
  const definition = {
