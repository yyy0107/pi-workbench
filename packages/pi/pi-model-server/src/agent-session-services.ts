...
function requestOptions<T extends { fetch?: FetchFunction }>(
  api: Api,
  ...
  const trace =
    typeof sessionId === "string" ? dependencies.getRequestObserver?.(sessionId) : undefined;
...
export async function createWorkbenchAgentSessionServices(
  ...
  runtime.stream = (model, context, options) =>
    stream(model, context, requestOptions(model.api, dependencies, options) as typeof options);
  runtime.streamSimple = (model, context, options) =>
    streamSimple(model, context, requestOptions(model.api, dependencies, options));
  runtime.fetchDeferred = (model, handle, options) =>
    fetchDeferred(model, handle, requestOptions(model.api, dependencies, options));
  runtime.cancelDeferred = (model, handle, options) =>
    cancelDeferred(model, handle, requestOptions(model.api, dependencies, options));
