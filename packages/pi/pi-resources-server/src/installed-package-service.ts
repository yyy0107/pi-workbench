import { ScopedResourceContextError } from "./scoped-resource-context";
import type { ScopedResourceContextService } from "./scoped-resource-context";
...
export type PackageSettingsResolver = (
  sessionId?: string,
) => Promise<{ cwd: string; settingsManager: SettingsManager }>;

export interface InstalledPackageServiceDependencies {
  resolvePackageSettings: PackageSettingsResolver;
...
export async function describeInstalledPackage(
  request: InstalledPackageDescribePayload,
  scopedResources: Pick<ScopedResourceContextService, "get">,
): Promise<InstalledPackageDetailsView> {
  const context = await scopedResources.get(request.target);
...
export async function userPackageSettings(
  getSession: (sessionId: string) => Promise<{
    session: { sessionManager: { getCwd(): string }; settingsManager: SettingsManager };
  }>,
  sessionId?: string,
): Promise<{
  cwd: string;
  ...
  if (sessionId) {
    const host = await getSession(sessionId);
    return {
...
export async function checkAvailablePackageUpdates(
  ...
  if ("target" in request && request.target) {
    ...
    const host = await getSession(request.sessionId);
    cwd = host.session.sessionManager.getCwd();
    settingsManager = host.session.settingsManager;
  }
...
async function installUserPackage(
  resolveSettings: PackageSettingsResolver,
  sessionId: string | undefined,
  source: string,
): Promise<void> {
  const { cwd, settingsManager } = await resolveSettings(sessionId);
  const packageManager = new DefaultPackageManager({
    cwd,
    agentDir: getAgentDir(),
    settingsManager,
...
export class InstalledPackageService implements InstalledPackageProtocol {
  ...
  constructor(
    dependencies: Pick<
      InstalledPackageServiceDependencies,
      | "resolvePackageSettings"
      | "getSession"
      | "getScopedResourceHost"
      | "getWorkspace"
      | "getLoadedSessions"
      | "describeInstalledPackage"
      | "checkAvailablePackageUpdates"
      | "reloadScopedResources"
      | "mutationCoordinator"
    > &
      Partial<InstalledPackageServiceDependencies>,
  ) {
    this.dependencies = {
      isProjectTrusted: (workspacePath) => getProjectTrustService().isTrusted(workspacePath),
      installUserPackage: (sessionId, source) =>
        installUserPackage(dependencies.resolvePackageSettings, sessionId, source),
      installProjectPackage,
      updateUserPackage: (sessionId, source) =>
        updateUserPackage(dependencies.resolvePackageSettings, sessionId, source),
      updateProjectPackage,
      prepareUserPackageRemoval: (sessionId, source) =>
        prepareUserPackageRemoval(dependencies.resolvePackageSettings, sessionId, source),
      prepareProjectPackageRemoval,
