...
import type { PiSessionSummary } from "@workbench/pi-protocol/messages";
import { AgentThreadStoreError } from "@workbench/agent-runtime-server/threads";

import { type PiAgentThreadStoreDependencies } from "@workbench/pi-session-server/threads";
import { createPiAgentThreadStore } from "../../src/session-composition";
...
function harness(overrides: Partial<PiAgentThreadStoreDependencies> = {}) {
  ...
  return { adapter: createPiAgentThreadStore(dependencies), calls };
}
...
test("projects the Pi session catalog and search index to neutral thread records", async () => {
  ...
  assert.deepEqual(await adapter.search!.listDocuments(), [
...
test("maps lifecycle operations to the existing Pi session registry", async () => {
  ...
  assert.deepEqual(await adapter.fork!.fork({ threadId: "chosen", atStateToken: "8" }), {
...
test("strictly validates opaque Pi event revision tokens", async () => {
  const { adapter, calls } = harness();

  for (const atStateToken of ["", "01", "-1", "1.5", "9007199254740992"]) {
    await assert.rejects(
      adapter.fork!.fork({ threadId: "chosen", atStateToken }),
      (error: unknown) => {
        assert.ok(error instanceof AgentThreadStoreError);
        assert.equal(error.code, "fork-unavailable");
        return true;
      },
    );
  }
  assert.equal(
    calls.some(({ name }) => name === "fork"),
    false,
  );

  const invalidRename = harness({ renameSession: async () => Number.NaN });
  await assert.rejects(
    invalidRename.adapter.rename({ threadId: "chosen", title: "Renamed" }),
    (error: unknown) => {
      assert.ok(error instanceof AgentThreadStoreError);
