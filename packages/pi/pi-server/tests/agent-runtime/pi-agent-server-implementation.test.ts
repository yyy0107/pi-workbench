...
import { createPiAgentServerImplementation } from "../../src/agent-runtime/pi-agent-server-implementation";
import {
...
test("binds the selected Pi host while creating the adapter", (t) => {
  ...
  createPiAgentServerImplementation({ host });
...
defineWorkbenchAgentServerAdapterContract({
  ...
  createHarness() {
    ...
    const execution = createPiAgentExecution({
    ...
    const threads = createPiAgentThreadStore({
    ...
    return {
      adapter: createPiAgentServerImplementation({ commands, execution, threads }),
