...
const { contextTraceExtension } = await import("../../src/tool-composition");
const { prepareWorkbenchPiExtensions } = await import("../../src/tool-composition");
const { activateSessionContextTrace, releaseSessionContextTrace } =
  await import("@workbench/pi-session-server/session-context-trace");
...
test("all builtin lifecycle extensions switch live without losing their catalog or persisted state", async (t) => {
  ...
  for (const key of [...Object.values(BUILTIN_EXTENSION_PREFERENCE_KEYS)]) {
    await settings.update({ patch: { [key]: false } });
    assert.equal(
