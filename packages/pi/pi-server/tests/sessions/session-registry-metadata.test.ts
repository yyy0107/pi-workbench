...
import type { PiImageContent } from "@workbench/pi-protocol/messages";
...
const { appendSessionEventJournal, initializeSessionEventJournal, SESSION_EVENT_CUSTOM_TYPE } =
  (await import("@workbench/pi-session-server/session-event-journal")) as typeof import("@workbench/pi-session-server/session-event-journal");
...
import { projectPiComposerContext } from "@workbench/pi-tools/composer-context";
...
import { getComposerTextAttachmentStore } from "@workbench/pi-session-server/composer-text-attachments";
...
test("keeps model selections session-local across prompts, cold reopen, and automation launches", async (t) => {
  ...
  const installedDefaults = JSON.parse(await readFile(settingsPath, "utf8"));
  for (const [key, value] of Object.entries(defaults)) assert.equal(installedDefaults[key], value);
  ...
  assert.deepEqual(JSON.parse(await readFile(settingsPath, "utf8")), installedDefaults);
  await first.shutdown();
  const reopened = await getOrStartSession(first.id);
  hosts.push(reopened);
...
test("cold history repairs a crashed first turn and the next hosted session sees the same checkpoint", async (t) => {
  ...
  const { ensureSessionPersistence } =
    await import("@workbench/pi-session-server/session-interruption");
