...
const MODEL_SERVICE = new URL("../../../pi-model-server/src/model-service.ts", import.meta.url);
const MODEL_PROVIDER_ROUTES = new URL(
  "../../src/routes/model-provider-rpc-routes.ts",
  import.meta.url,
);
const MODEL_CONTEXT_WINDOW_ROUTES = new URL(
  "../../src/routes/model-context-window-rpc-routes.ts",
  import.meta.url,
);
...
test("ModelService implements both protocols while retaining Pi runtime and credential ownership", async () => {
  ...
  assert.match(source, /createWorkbenchAgentSessionServices/);
...
test("the route composition creates model groups without retaining LLM transport details", async () => {
  ...
  assert.match(source, /resolveProjectTrust/);
  assert.match(source, /getRequestObserver/);
