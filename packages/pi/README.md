...
Pi 按具体能力组织为 `packages/pi/<capability>`。每包同时拥有真实能力源码 `src/`、内部辅助源码 `lib/` 和 `tests/`；两处源码均最多一级子目录，保留 TS/TSX。跨包只使用显式 exports。

| 能力       | 目录                                                                            | 职责                                                |
| ---------- | ------------------------------------------------------------------------------- | --------------------------------------------------- |
| 客户端安装 | `client`                                                                        | Runtime、线程与产品 API                             |
| 客户端连接 | `transport-client`                                                              | HTTP/RPC、连接状态、双 WebSocket                    |
| 消息       | `conversation`                                                                  | 纯解析、投影、累积器和组装器                        |
| 协议与共享 | `protocol`、`shared`                                                            | RPC/流帧与跨端纯契约                                |
| 服务端端口 | `server-ports`                                                                  | Host、publisher、会话访问与工具协作接口             |
| 模型       | `model-server`                                                                  | 配置、认证和 SDK model services                     |
| 资源       | `resources-server`                                                              | Skills、Extensions、Packages、Prompts、设置和工作区 |
| 工具       | `tools`、`browser`                                                              | 内置工具工厂与浏览器工具                            |
| 会话       | `session-server`                                                                | SDK 会话注册、历史、附件、导入、执行和 Trace        |
| 服务端装配 | `server`                                                                        | 安装、HTTP/WS、RPC 路由，选择并注入原有共享实例     |
| 界面       | `settings-ui`、`toolbox-ui`、`diagnostics-ui`、`status-ui`、`session-import-ui` | 各能力的组件、扩展、词典与辅助模块                  |
| 界面安装   | `contributions`                                                                 | 稳定的扩展及翻译 bundle 安装清单                    |
