...
源码分工：src 承载本包能力与契约，lib 仅放实际使用的内部辅助，tests 为包根测试。实际消费者示例：`src/pi-running-indicator.tsx` 引用 `lib/wordmark-pixels.ts`。实现保留 TS/TSX；既有构建工具保持原语言。
