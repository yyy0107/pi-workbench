...
import { REVIEW_ENTRY_TYPE, getReviewSnapshots } from "@workbench/pi-tools/workspace-review";
...
import { createWorkbenchAgentSessionServices as createModelServices } from "@workbench/pi-model-server/services";
import { withWorkbenchBuiltinSkills } from "@workbench/pi-resources-server/builtin-skills";
import { requireSkillOptIn } from "@workbench/pi-resources-server/skill-enablement";
import { workbenchToolOverrides } from "@workbench/pi-tools/builtin-tools";
import type {
  PiAssistantMessage,
  PiAgentMessage,
  PiEvent,
...
} from "@workbench/pi-shared/composer-prompt";
import { PI_MODEL_CHANGED_EVENT, PI_SESSION_FORKED_EVENT } from "@workbench/pi-protocol/messages";
import { PI_CANCEL_INTENT_CUSTOM_TYPE } from "@workbench/pi-shared/messages";
import type {
  SessionCompactValue,
...
import { deriveSessionDisplayTitle } from "@workbench/pi-shared/sessions";
import {
...
import { applySessionMessageDelta, copyPiAssistantMessage } from "@workbench/pi-shared/messages";
import {
...
} from "@workbench/pi-resources-server/composer-command-planner";
import { expandPromptTemplateContent } from "@workbench/pi-resources-server/prompt-template-expander";
import { composerCommandFailureReason } from "@workbench/pi-resources-server/composer-command-failure";
import {
  piCompactUsesLegacyArguments,
  resolvePiCompactCustomInstructions,
} from "@workbench/pi-resources-server/pi-composer-command-arguments";
import { PiServerError } from "@workbench/pi-server-ports/errors";
...
import {
  validateWorkspace,
  workspaceFromCwd,
} from "@workbench/pi-resources-server/workspace-paths";
...
type HostedPiSession = HostedSession;
...
export function createPiSessionRegistry(dependencies: PiSessionRuntimeDependencies) {
  ...
  function compactAssistantMessageUpdate(event: PiEvent): SessionMessageDelta | undefined {
  ...
  function sessionModifiedAt(source: SessionTimestampSource): Date {
  ...
  function textOnlyModelContext(messages: readonly unknown[]): readonly unknown[] {
    let imageSequence = 0;
    let changed = false;
  ...
  class HostedPiSession {
    ...
    private flushAssistantMessageChunk(stream = this.activeAssistantStream): void {
      ...
      const canonical = this.appendCanonicalEvent(
        { type: "message_update", ...chunk },
        pending.time,
      );
      if (!canonical) return;
        ...
        try {
          getStreamHub().publishHost({
            ...
            message:
              error instanceof Error
                ? error.message
                : "The recovery checkpoint could not be saved.",
          });
        } catch {
    ...
    private contextualizeModel<
      Model extends { provider: string; id: string; contextWindow: number },
    >(model: Model): Model {
    ...
    regenerate(messageId: string, requestId?: string): Promise<void> {
      return this.runQueueMutation(async () => {
        ...
        const userEntry = (() => {
          ...
          if (entry?.type !== "message" || !jsonEqual(entry.message, eventMessage))
            return undefined;
          legacySource = true;
          return entry;
        })();
      ...
      return this.runQueueMutation(async () => {
        ...
        while (resumableMessages.length > 0) {
          ...
          if (
            !isRecord(tail) ||
            tail.role !== "assistant" ||
            (resumeReasonFromAssistantMessage(tail as unknown as PiAssistantMessage) ===
              undefined &&
    ...
    private async resolveComposerSubmission(
        ...
        await Promise.all(
          imageAttachments.map(
            async (attachment) =>
              [
                attachment.id,
                (await attachmentStore.readImage({ id: attachment.id })).data,
              ] as const,
          ),
        ),
      );
      ...
      const commandOwnsAgentTurn = plannedCommands.some(
        (command) => command.effect === "agent-turn",
      );
      const needsMainTurn =
  ...
  function publishRunningSessions(): void {
    ...
    const previous = new Set(
      registry.lastRunningKey ? registry.lastRunningKey.split("\u0000") : [],
    );
  ...
  function startPersistedSessionCacheRefresh(
    ...
    const task = (async () => {
      let indexStatus: "memory" | "hit" | "missing" | "invalid" =
        registry.persistedSessionCacheReady ? "memory" : "missing";
