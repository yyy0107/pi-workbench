...
import { createSessionMessageChunkData } from "@workbench/pi-protocol/stream";
import type { SessionEvent } from "@workbench/pi-protocol/rpc";
import { ensureSessionPersistence, reconcileInterruptedSession } from "../src/session-interruption";
import {
  appendSessionEventJournal,
  initializeSessionEventJournal,
  readSessionEventJournal,
} from "../src/session-event-journal";
import {
...
test("recovers the first streamed answer after its writer process is forcibly killed", (t) => {
  ...
  const child = spawnSync(
    ...
    [
      ...
      import { ensureSessionPersistence } from ${JSON.stringify(new URL("../src/session-interruption.ts", import.meta.url).href)};
      import { appendSessionEventJournal, initializeSessionEventJournal } from ${JSON.stringify(new URL("../src/session-event-journal.ts", import.meta.url).href)};
