...
import type { ServerRequest } from "@workbench/agent-runtime-pi-protocol/stream";
import type {
  DownlinkWebSocket,
  UpgradeRequestLike,
  UpgradeSocketLike,
  WebSocketGatewayTimers,
} from "../../src/streams/websocket-gateway";

const { acceptDownlinkWebSocket, createNoServerWebSocketGateway, streamNameForUpgradeUrl } =
  (await import(
    new URL("../../src/streams/websocket-gateway.ts", import.meta.url).href
  )) as typeof import("../../src/streams/websocket-gateway");
...
class FakeTimers implements WebSocketGatewayTimers {
  private nextId = 0;
...
test("waits for the backpressure grace period before closing a stalled client", async () => {
  const hub = createStreamHub({ createRpcId: () => "rpc-slow" });
  const socket = new FakeWebSocket();
  const timers = new FakeTimers();
...
test("keeps a healthy burst connected when queued writes make progress", async () => {
  let nextId = 0;
