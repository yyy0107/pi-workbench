...
const { reportWorkbenchInternalPiExtensionErrors } = await import("@workbench/pi-tools");
const { workbenchInternalPiExtensions } = await import("../../src/tool-composition");
const { messageTerminationExtension } = await import("@workbench/pi-tools/message-termination");
...
test("registers Workbench-owned adapters as hidden inline extensions", () => {
  assert.deepEqual(
    ...
    [
      ...
      { name: "workbench.workspace-review", hidden: true },
