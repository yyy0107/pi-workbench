...
import type { PiPackageUpdatesValue } from "@workbench/agent-runtime-pi-protocol/rpc";
...
} from "@workbench/pi-resources-server/packages";
import { InstalledPackageService } from "../../src/resource-composition";

import { WORKBENCH_BROWSER_PACKAGE_SOURCE } from "@workbench/pi-resources-server/builtin-packages";
...
test("identifies the built-in browser package and rejects its removal or replacement", async () => {
  ...
  assert.deepEqual(await service.list({ target }), {
    packages: [
      { source, scope: "user", filtered: true, builtin: true, name: "@workbench/pi-browser" },
    ],
  });
