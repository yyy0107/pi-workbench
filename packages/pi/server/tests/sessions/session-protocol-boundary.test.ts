...
const SESSION_RPC_SERVICE = new URL(
  "../../../session-server/src/session-rpc-service.ts",
  import.meta.url,
);
const SESSION_PROTOCOL_FACADE = new URL(
  "../../../session-server/src/pi-session-protocol-facade.ts",
  import.meta.url,
);
const SESSION_CONTEXT_TRACE_SERVICE = new URL(
  "../../../session-server/src/pi-session-context-trace-service.ts",
  import.meta.url,
);
const EXTERNAL_SESSION_IMPORT_SERVICE = new URL(
  "../../../session-server/src/external-session-import-service.ts",
  import.meta.url,
);
const EXTERNAL_SESSION_TYPES = new URL(
  "../../../session-server/src/external-session-types.ts",
  import.meta.url,
);
...
const RPC_ROUTE_GROUP = new URL("../../../../host/server/src/rpc.ts", import.meta.url);
...
const EXTERNAL_SESSION_IMPORT_RPC_ROUTES = new URL(
  "../../src/routes/external-session-import-rpc-routes.ts",
  import.meta.url,
);
...
test("the session protocol facade owns its service and accepts composed collaborators with late-bound workspaces", async () => {
  const source = await readFile(SESSION_PROTOCOL_FACADE, "utf8");

  assert.doesNotMatch(source, /createPiAgentServerImplementation/);
  assert.match(source, /history: options\.history/);
  assert.match(source, /modelContext: options\.modelContext/);
  const composition = await readFile(
    new URL("../../src/session-composition.ts", import.meta.url),
    "utf8",
  );
  assert.match(composition, /history: options\.history \?\? createPiSessionHistoryService\(\)/);
  assert.match(
    composition,
...
test("the Pi Context Trace service owns live-session and journal coordination", async () => {
  ...
  assert.doesNotMatch(source, /from\s+["']\.\/session-registry["']/);
  assert.match(source, /defaults:\s*PiSessionContextTraceServiceDependencies/);
  ...
  assert.match(composition, /readPromptParts: readSessionContextTracePromptParts/);
...
test("external import domain types reuse the stable browser-facing RPC contract", async () => {
  ...
  assert.match(source, /from\s+["']@workbench\/agent-runtime-pi-protocol\/rpc["']/);
...
test("the external import service owns Pi persistence, source importers, and Workspace attachment", async () => {
  ...
  assert.match(source, /new CodexSessionImporter\(\)/);
  assert.match(source, /new ClaudeCodeSessionImporter\(\)/);
  assert.match(source, /new CursorSessionImporter\(\)/);
...
test("external import transport routes depend only on the narrow import protocol", async () => {
  ...
  assert.doesNotMatch(
    source,
    /CodexSessionImporter|ClaudeCodeSessionImporter|CursorSessionImporter/,
  );
