...
import { MAX_SKILL_DOCUMENT_BYTES, SkillServiceError } from "@workbench/pi-resources-server/skills";
...
test("lists the skills loaded by the target Pi session", async (t) => {
  const agentDir = await mkdtemp(path.join(tmpdir(), "isolated-skill-catalog-"));
...
test("lists a Toolbox scope without resolving a Pi session", async (t) => {
  const agentDir = await mkdtemp(path.join(tmpdir(), "isolated-skill-catalog-"));
  t.after(() => rm(agentDir, { recursive: true, force: true }));
