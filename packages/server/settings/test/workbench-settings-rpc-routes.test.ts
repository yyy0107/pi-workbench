  ...
  const update = routes.handle(
    rpcRequest("workbenchSettings.update", {
      patch: {
        ...
        editToolEnabled: false,
        writeToolEnabled: false,
        grepToolEnabled: true,
        findToolEnabled: true,
        lsToolEnabled: true,
        retainAllModelIO: true,
        showTodos: false,
        groupExplorationTools: false,
        groupTerminalTools: false,
...
test("validates Workbench Settings patches before resolving a service", async () => {
  ...
  for (const payload of [
    {},
    { patch: { locale: "en" } },
    { patch: { runningMessageMode: "invalid" } },
    { patch: { showReasoning: "false" } },
    { patch: { groupParallelTools: 1 } },
