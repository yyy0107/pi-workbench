...
import { WorkspaceStoreError, WorkspaceStore } from "../src/catalog";

type WorkspaceStoreEvent = import("@workbench/workspace-server/catalog").WorkspaceStoreEvent;
...
async function expectStoreError(
  ...
  code: import("@workbench/workspace-server/catalog").WorkspaceStoreErrorCode,
  details: object,
): Promise<void> {
...
test("reuses persisted workspace identities after their paths become symlink aliases", async (t) => {
  for (const operation of ["create", "reconcile", "unarchive"] as const) {
    ...
    assert.ok(!events.some((event) => event.type === "workspace-order-changed"));
  }
});
...
test("prepends attached sessions, reorders them locally, and archives globally", async (t) => {
  ...
  assert.deepEqual(
    events
      .slice(-4)
      .map((event) =>
        event.type === "session-archive-changed"
...
test("persists workspace and session pins and publishes authoritative changes", async (t) => {
  ...
  assert.deepEqual(
    events.map((event) => event.type),
    ["workspace-pinned-changed", "session-pinned-changed"],
  );
});
...
test("unarchive restores an unassigned session to its canonical workspace atomically", async (t) => {
  ...
  assert.equal(events[0]?.type, "session-archive-changed");
  if (events[0]?.type !== "session-archive-changed") {
...
test("explicit session removal clears workspace and archive metadata", async (t) => {
  ...
  assert.deepEqual(
    events.map((event) => event.type),
    ["workspace-changed", "session-archive-changed"],
  );
});
