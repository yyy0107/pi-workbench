...
import { WorkspaceFileError, WorkspaceFileService } from "../src/files";
...
async function fixture(t: test.TestContext) {
  ...
  const workspace = { workspaceId: "workspace-1", path: workspaceRoot };
  const files = new WorkspaceFileService({
    resolveWorkspaceRoot: async (id) => (id === workspace.workspaceId ? workspaceRoot : undefined),
  });
