import assert from "node:assert/strict";
import { mkdtemp, rm } from "node:fs/promises";
import { tmpdir } from "node:os";
import path from "node:path";
import test from "node:test";

import type { WorkflowDocument, WorkflowRunStatus } from "@workbench/execution-contracts";
import { ExecutionNodeExecutorRegistry, type ExecutionNodeExecutor } from "../src/node-executor";
import { compileExecutionDocument } from "../src/compiler";
import { ExecutionEngine } from "../src/engine";
import { ExecutionRepository } from "../src/repository";

function approvalDocument(): WorkflowDocument {
  return {
    schemaVersion: 3,
    id: "approval-flow",
    kind: "workflow",
    scope: { type: "personal" },
    name: "Approval flow",
    agents: [],
    graph: {
      nodes: [
        { id: "start", type: "start", name: "Start", position: { x: 0, y: 0 }, config: {} },
        {
          id: "approval",
          type: "approval",
          name: "Approval",
          position: { x: 100, y: 0 },
          config: { message: "Approve?" },
        },
        { id: "end", type: "end", name: "End", position: { x: 200, y: 0 }, config: {} },
      ],
      edges: [
        { id: "a", source: "start", target: "approval" },
        { id: "b", source: "approval", target: "end" },
      ],
      editor: {},
    },
    concurrency: { mode: "queue" },
    draftRevision: 0,
    createdAt: 1,
    updatedAt: 1,
  };
}

function parallelDocument(): WorkflowDocument {
  return {
    ...approvalDocument(),
    id: "parallel-flow",
    kind: "workflow",
    name: "Parallel flow",
    agents: [
      { id: "left-agent", name: "Left" },
      { id: "right-agent", name: "Right" },
    ],
    graph: {
      nodes: [
        { id: "start", type: "start", name: "Start", position: { x: 0, y: 0 }, config: {} },
        {
          id: "left",
          type: "agent",
          name: "Left",
          position: { x: 100, y: 0 },
          config: { agentId: "left-agent", promptTemplate: "default", output: { schema: {} } },
        },
        {
          id: "right",
          type: "agent",
          name: "Right",
          position: { x: 100, y: 100 },
          config: { agentId: "right-agent", promptTemplate: "default", output: { schema: {} } },
        },
        { id: "end", type: "end", name: "End", position: { x: 200, y: 0 }, config: {} },
      ],
      edges: [
        { id: "left-in", source: "start", target: "left" },
        { id: "right-in", source: "start", target: "right" },
        { id: "left-out", source: "left", target: "end" },
        { id: "right-out", source: "right", target: "end" },
      ],
      editor: {},
    },
  };
}

function commandDocument(): WorkflowDocument {
  return {
    ...approvalDocument(),
    id: "command-flow",
    name: "Command flow",
    graph: {
      nodes: [
        { id: "start", type: "start", name: "Start", position: { x: 0, y: 0 }, config: {} },
        {
          id: "command",
          type: "command",
          name: "Command",
          position: { x: 100, y: 0 },
          config: { command: "true" },
        },
        { id: "end", type: "end", name: "End", position: { x: 200, y: 0 }, config: {} },
      ],
      edges: [
        { id: "command-in", source: "start", target: "command" },
        { id: "command-out", source: "command", target: "end" },
      ],
      editor: {},
    },
  };
}

async function waitForStatus(
  repository: ExecutionRepository,
  runId: string,
  status: WorkflowRunStatus,
): Promise<void> {
  for (let attempt = 0; attempt < 100; attempt += 1) {
    if ((await repository.readRunSummary(runId)).status === status) return;
    await new Promise((resolve) => setTimeout(resolve, 5));
  }
  assert.fail(`Run ${runId} did not reach ${status}.`);
}

test("pauses an Approval node and resumes the same run after resolution", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "workbench-flow-engine-"));
  try {
    const repository = new ExecutionRepository({
      rootDirectory: root,
      listWorkspaces: async () => [],
    });
    const inert: ExecutionNodeExecutor = {
      async execute() {
        return {};
      },
    };
    const engine = new ExecutionEngine({
      repository,
      executors: new ExecutionNodeExecutorRegistry({ agent: inert, command: inert }),
    });
    const revision = compileExecutionDocument(approvalDocument(), 10).revision;
    const admission = await engine.start({ revision, source: "manual" });
    assert.notEqual(admission.kind, "skipped");
    if (admission.kind === "skipped") return;
    await waitForStatus(repository, admission.run.id, "waiting-for-approval");
    await engine.resolveApproval({
      runId: admission.run.id,
      nodeId: "approval",
      approved: true,
      result: { approvedBy: "test" },
    });
    await waitForStatus(repository, admission.run.id, "succeeded");
    const run = await repository.readRunSummary(admission.run.id);
    assert.equal(run.attempts.find(({ nodeId }) => nodeId === "approval")?.status, "succeeded");
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

test("reports running only after an execution slot is acquired and queued only when blocked", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "workbench-flow-engine-admission-"));
  try {
    const repository = new ExecutionRepository({
      rootDirectory: root,
      listWorkspaces: async () => [],
    });
    const inert: ExecutionNodeExecutor = {
      async execute() {
        return {};
      },
    };
    const changedStatuses: WorkflowRunStatus[] = [];
    const engine = new ExecutionEngine({
      repository,
      executors: new ExecutionNodeExecutorRegistry({ agent: inert, command: inert }),
      onRunChanged: (run) => changedStatuses.push(run.status),
    });
    const revision = compileExecutionDocument(approvalDocument(), 10).revision;

    const started = await engine.start({ revision, source: "manual" });
    assert.equal(started.kind, "started");
    if (started.kind !== "started") return;
    assert.equal(started.run.status, "running");
    assert.equal(typeof started.run.startedAt, "number");
    assert.equal(changedStatuses[0], "running");

    const queued = await engine.start({ revision, source: "manual" });
    assert.equal(queued.kind, "queued");
    assert.equal(queued.run.status, "queued");
    assert.equal(queued.run.startedAt, undefined);
    assert.equal(changedStatuses.at(-1), "queued");

    await engine.cancel(queued.run.id);
    await engine.cancel(started.run.id);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

test("records every settled parallel node before propagating a branch failure", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "workbench-flow-engine-parallel-"));
  try {
    const repository = new ExecutionRepository({
      rootDirectory: root,
      listWorkspaces: async () => [],
    });
    const executor: ExecutionNodeExecutor = {
      async execute({ node }) {
        if (node.id === "left") {
          const error = new Error("left failed");
          Object.assign(error, { code: "left-failed" });
          throw error;
        }
        return { output: { branch: node.id } };
      },
    };
    const engine = new ExecutionEngine({
      repository,
      executors: new ExecutionNodeExecutorRegistry({ agent: executor, command: executor }),
    });
    const admission = await engine.start({
      revision: compileExecutionDocument(parallelDocument(), 10).revision,
      source: "manual",
    });
    assert.notEqual(admission.kind, "skipped");
    if (admission.kind === "skipped") return;
    await waitForStatus(repository, admission.run.id, "failed");
    const run = await repository.readRunSummary(admission.run.id);
    assert.equal(run.attempts.find(({ nodeId }) => nodeId === "left")?.status, "failed");
    assert.equal(run.attempts.find(({ nodeId }) => nodeId === "right")?.status, "succeeded");
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

test("shutdown aborts active work, cancels queued runs, and never drains after admission closes", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "workbench-flow-engine-shutdown-"));
  try {
    const repository = new ExecutionRepository({
      rootDirectory: root,
      listWorkspaces: async () => [],
    });
    let executions = 0;
    let activeSignal: AbortSignal | undefined;
    let reenteredShutdown: Promise<void> | undefined;
    let engine!: ExecutionEngine;
    let began!: () => void;
    const executing = new Promise<void>((resolve) => (began = resolve));
    const blocking: ExecutionNodeExecutor = {
      async execute(context) {
        executions += 1;
        activeSignal = context.signal;
        began();
        return await new Promise<never>((_resolve, reject) => {
          const abort = () => reject(new DOMException("Run cancelled", "AbortError"));
          const abortAndReenter = () => {
            reenteredShutdown = engine.shutdown();
            abort();
          };
          if (context.signal.aborted) abortAndReenter();
          else context.signal.addEventListener("abort", abortAndReenter, { once: true });
        });
      },
    };
    engine = new ExecutionEngine({
      repository,
      executors: new ExecutionNodeExecutorRegistry({ agent: blocking, command: blocking }),
    });
    const revision = compileExecutionDocument(commandDocument(), 10).revision;
    const started = await engine.start({ revision, source: "manual" });
    assert.equal(started.kind, "started");
    if (started.kind !== "started") return;
    await executing;
    const queued = await engine.start({ revision, source: "manual" });
    assert.equal(queued.kind, "queued");
    if (queued.kind !== "queued") return;

    const shutdown = engine.shutdown();
    assert.equal(engine.shutdown(), shutdown, "shutdown must be referentially idempotent");
    assert.equal(
      reenteredShutdown,
      shutdown,
      "synchronous abort re-entry must observe one operation",
    );
    await shutdown;

    assert.equal(activeSignal?.aborted, true);
    assert.equal(executions, 1, "a queued run must not allocate an executor during shutdown");
    assert.equal((await repository.readRunSummary(started.run.id)).status, "cancelled");
    const queuedRun = await repository.readRunSummary(queued.run.id);
    assert.equal(queuedRun.status, "cancelled");
    assert.deepEqual(queuedRun.attempts, []);
    await assert.rejects(engine.start({ revision, source: "manual" }), /shutting down/u);
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});

test("shutdown releases a pending approval and persists the run as cancelled", async () => {
  const root = await mkdtemp(path.join(tmpdir(), "workbench-flow-engine-approval-shutdown-"));
  try {
    const repository = new ExecutionRepository({
      rootDirectory: root,
      listWorkspaces: async () => [],
    });
    const inert: ExecutionNodeExecutor = {
      async execute() {
        return {};
      },
    };
    const engine = new ExecutionEngine({
      repository,
      executors: new ExecutionNodeExecutorRegistry({ agent: inert, command: inert }),
    });
    const revision = compileExecutionDocument(approvalDocument(), 10).revision;
    const admission = await engine.start({ revision, source: "manual" });
    assert.notEqual(admission.kind, "skipped");
    if (admission.kind === "skipped") return;
    await waitForStatus(repository, admission.run.id, "waiting-for-approval");

    await engine.shutdown();

    const run = await repository.readRunSummary(admission.run.id);
    assert.equal(run.status, "cancelled");
    assert.equal(run.attempts.find(({ nodeId }) => nodeId === "approval")?.status, "cancelled");
    await assert.rejects(
      engine.resolveApproval({
        runId: admission.run.id,
        nodeId: "approval",
        approved: true,
      }),
      (error: unknown) =>
        error instanceof Error && "code" in error && error.code === "approval-not-found",
    );
  } finally {
    await rm(root, { recursive: true, force: true });
  }
});
