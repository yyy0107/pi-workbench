...
import { ExecutionNodeExecutorRegistry, type ExecutionNodeResult } from "./node-executor";
import { ExecutionError } from "./errors";
import { compileExecutionRevision, type CompiledExecutionPlan } from "./compiler";
...
type ExecutionEngineLifecycleState = "running" | "stopping" | "stopped";
...
export class ExecutionEngine {
  ...
  // These fields are optional so a process-global engine retained by development HMR can lazily
  // acquire the lifecycle state introduced by a newer module generation without losing its runs.
  private lifecycle?: ExecutionEngineLifecycleState = "running";
  private pendingAdmissions?: Set<Promise<void>> = new Set();
  ...
  private lifecycleState(): ExecutionEngineLifecycleState {
    return (this.lifecycle ??= "running");
  }

  private admissionSettlements(): Set<Promise<void>> {
    return (this.pendingAdmissions ??= new Set());
  }
  ...
  async start(input: StartExecutionInput): Promise<WorkflowRunAdmission> {
    ...
    this.admissionSettlements().add(admissionSettled);
    try {
      ...
      if (this.lifecycleState() !== "running") {
        await this.persistRunStatus(persisted, "cancelled", {
          completedAt: this.now(),
          errorCode: "run-cancelled",
          errorMessage: "Workbench shut down before the run was admitted.",
        });
        throw new Error("Execution engine is shutting down.");
      }
      const queued = {
      ...
      this.admissionSettlements().delete(admissionSettled);
      settleAdmission();
    }
  }
  ...
  /** Stops admission, cancels queued/active work, and settles every owned run exactly once. */
  shutdown(): Promise<void> {
    ...
    const shutdownOperation = new Promise<void>((resolve, reject) => {
      resolveShutdown = resolve;
      rejectShutdown = reject;
    });
    // Publish the stable operation before abort dispatch: AbortSignal listeners run synchronously
    // and are allowed to re-enter shutdown through an owning service or application hook.
    this.shutdownOperation = shutdownOperation;
    const pendingAdmissions = [...this.admissionSettlements()];
    const queued = [...this.queuedByWorkflow.values()].flat();
    this.queuedByWorkflow.clear();
    ...
    void (async () => {
      const results = await Promise.allSettled([
        ...pendingAdmissions,
        ...queued.map(({ run }) =>
          this.persistRunStatus(run, "cancelled", {
            completedAt: this.now(),
            errorCode: "run-cancelled",
            errorMessage: "Workbench shut down before the run started.",
          }),
        ),
        ...active.map(({ done }) => done),
      ]);
      const errors = results.flatMap((result) =>
        result.status === "rejected" ? [result.reason] : [],
      );
      if (errors.length > 0) {
        throw new AggregateError(errors, "Execution engine shutdown failed.");
      }
    })().then(
      () => {
