...
const AUTOMATION_ROOTS = [
  ...
  "packages/server/automation/src",
];
...
test("Automation and Workflow production domains remain mutually independent", async () => {
  ...
  for (const file of await sources(AUTOMATION_ROOTS)) {
    ...
    if (
      /(?:@workbench\/(?:execution-contracts|execution-server)|packages\/(?:server\/execution\/src|agent-runtime\/adapters\/pi\/(?:client\/src\/workflows|server\/src\/executions)))/u.test(
        content,
      )
    ) {
  ...
  for (const file of await sources(WORKFLOW_ROOTS)) {
    const content = await readFile(file, "utf8");
    if (
      /(?:@workbench\/(?:automation-contracts|automation-server)|packages\/(?:server\/automation\/src|agent-runtime\/adapters\/pi\/(?:client\/src\/automations|server\/src\/automations)))/u.test(
        content,
      )
    ) {
