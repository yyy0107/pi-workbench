...
test("a language chosen during hydration takes precedence over the saved locale", async () => {
  const dom = installMinimalReactDomEnvironment();
  const root = createRoot(dom.container);
  let resolveLoad!: (value: WorkbenchSettingsPreferences) => void;
  const pendingLoad = new Promise<WorkbenchSettingsPreferences>((resolve) => {
    resolveLoad = resolve;
  });
  ...
  try {
    ...
    assert.deepEqual(writes, [{ locale: "zh-CN" }]);
  } finally {
    await act(async () => {
      root.unmount();
    });
