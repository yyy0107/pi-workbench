...
import { WorkbenchPortalContainerProvider, useWorkbenchPortalContainer } from "@workbench/ui";
...
test("every Shell floating primitive targets its installation portal container", async () => {
  ...
  for (const filename of portalSources) {
    const source = await readFile(
      new URL(`./${filename}`, new URL("../../../../client/ui/src/components/", import.meta.url)),
      "utf8",
    );
  ...
  const [citation, workspaceTabs, workbenchShell] = await Promise.all([
    readFile(
      new URL("../../../../client/markdown/src/inline-citation.tsx", import.meta.url),
      "utf8",
    ),
    readFile(
    ...
    readFile(new URL("../../src/shell/workbench-shell.tsx", import.meta.url), "utf8"),
  ]);
