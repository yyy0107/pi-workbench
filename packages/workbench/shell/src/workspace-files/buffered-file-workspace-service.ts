...
export interface FileWorkspaceBackend {
  contentUrl?(request: WorkbenchWorkspaceFileRequest): string | undefined;
  listDirectory(
...
/** Runtime-owned resource access, projected into the same file buffer contract. */
export interface FileWorkspaceResourceBackend {
...
export function workspaceAbsolutePath(rootPath: string | undefined, path: string): string {
...
function snapshotFromRemote(value: WorkbenchWorkspaceFileSnapshot): FileSnapshot {
  return {
  ...
  async listDirectory(context: WorkspaceFileContext, path: string): Promise<FileDirectoryListing> {
    ...
    if (context.session && context.session.source !== "workspace") {
      ...
      return this.#resources.listDirectory(
        context.session,
        workspaceRelativePath(context.rootPath, path),
      );
    }
