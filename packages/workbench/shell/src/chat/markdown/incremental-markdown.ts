// Adapted from DeepSeek Harness (c291e796), packages/client/ui-primitives/src/markdown/incremental.ts.
// Copyright (c) 2026 DeepSeek. MIT license; see DEEPSEEK-LICENSE.txt.
/**
...
export class IncrementalMarkdownParser {
  ...
  private readonly parse: (text: string) => Root;
  ...
  private fenceValue(
    state: Pick<OpenFenceState, "syntheticPrefix">,
    text: string,
  ): string | undefined {
  ...
  private openFenceState(
    ...
    const valuePrefix =
      stableLength === 0 ? "" : `${stableValue}${trailingLineTerminator(stableSource)}`;
  ...
  private updateOpenFence(
    ...
    const stableValue =
      stableLength === 0 ? "" : this.fenceValue(state, pending.slice(0, stableLength));
    ...
    const tail = state.tail.map((entry, index) =>
      index === state.codeIndex ? { ...entry, node } : entry,
    );
    const cached = {
      frozen: state.frozen,
      tail,
