...
function AvailableAttachmentUnderstandingSettingsItem({
  sectionId,
  itemId,
  attachments,
  models,
}: SettingsItemComponentProps & {
  attachments: WorkbenchAttachmentUnderstandingCapability;
  ...
  const ocrPollIntervalId = `${domScopeId}-attachment-ocr-adapter-poll-interval`;
  const ocrPollTimeoutId = `${domScopeId}-attachment-ocr-adapter-poll-timeout`;
  const ocrSourceId = `${domScopeId}-attachment-ocr-adapter-source`;
  const multimodalProviderId = `${domScopeId}-image-multimodal-provider`;
  const multimodalModelId = `${domScopeId}-image-multimodal-model`;
  ...
  const modelCatalogRevision = useSyncExternalStore(
    models.subscribeCatalog,
    models.getCatalogRevision,
    models.getCatalogRevision,
  );
  ...
  const load = useCallback(() => {
    ...
    void attachments.describe().then(
      (next) => {
  ...
  const loadModelCatalog = useCallback(() => {
    ...
    void models.listCatalog({ configuredOnly: true }).then(
      (catalog) => {
        if (request !== modelCatalogRequestRef.current) return;
        setMultimodalProviders(configuredMultimodalModelOptions(catalog));
  ...
  const saveRouting = useCallback(
    ...
    [attachments, draft, saving, t, view],
  );
  ...
  }, [attachments, draft, saving, t, validationError, view]);
            ...
            <div className="sm:col-span-2">
              ...
              >
                <div className="flex gap-2">
                  <Input
                    id={ocrApiKeyId}
                    aria-describedby={`${ocrApiKeyId}-description`}
                    type="password"
            ...
            <div className="sm:col-span-2">
              ...
              >
                <WorkbenchCodeEditor
                  ...
                  disabled={ocrAdapterDisabled}
                  className="bg-background h-96 min-h-64 max-h-[36rem] resize-y rounded-lg border"
                  onSave={save}
                  onChange={(source) => {
...
export function AttachmentUnderstandingSettingsItem(props: SettingsItemComponentProps) {
  ...
  return attachments && models ? (
    <AvailableAttachmentUnderstandingSettingsItem
      {...props}
