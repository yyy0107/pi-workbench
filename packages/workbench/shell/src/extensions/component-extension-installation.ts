...
/**
 * Component-extension enablement is deliberately a realm-wide persisted preference, not mutable
 * state owned by one WorkbenchShell installation. Every Shell mounted in the same browser realm
 * observes the same origin storage key and storage events; the module cache only stabilizes those
 * shared snapshots and supplies a realm-wide fallback when localStorage is unavailable.
 */
