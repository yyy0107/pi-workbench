...
- `server/src/agent-runtime` 实现 `@workbench/agent-runtime-server` 的后端无关执行与线程存储端口，把 `threadId`、
  ...
  `AgentCommandCatalogPort`。`pi-agent-server-implementation.test.ts` 以可注入的底层依赖运行通用
  `defineWorkbenchAgentServerAdapterContract()`；测试仍经过真实 Pi execution/thread implementation，不创建
  第二个 session service；应用组合根直接创建唯一的 Pi implementation 并绑定 Host，默认服务图保持不变；
- `server/src/sessions/pi-session-protocol-facade.ts` 是 Pi session protocol 的组合根：每个 server module
  ...
  无需退化成请求级对象图；
- `server/src/sessions/pi-session-history-service.ts` 拥有 Pi canonical event 的完整消息组分页、历史搜索
...
- `server/src/transport` 只处理 carrier、信任、校验、路由和 upgrade。`routes/session-rpc-routes.ts`
...
- `server/src/extensions/extension-service.ts` 实现窄的 `ExtensionProtocol`，并继续独占 Pi ResourceLoader、
  SettingsManager、已加载扩展 runtime 映射、session/scoped resource host、启停与删除协调，以及扩展
  授权目录和文件读取；transport 只接收稳定、可序列化的协议输入与输出；
- `server/src/packages/installed-package-service.ts` 实现窄的 `InstalledPackageProtocol`，继续独占 Pi
...
- `server/src/settings/agent-settings-service.ts` 实现 `AgentSettingsProtocol`，继续独占 Pi agent 目录、
...
- `server/src/host/host-service.ts` 实现 `HostProtocol`，继续独占 Pi/Workbench 版本、当前模型、已附加会话、
...
- `server/src/commands/command-service.ts` 的 `CommandCatalogProtocol` 保留 session/target 兼容 wire DTO，并与
