...
export function FileBreadcrumbTree({ surface, context }: WorkspaceSurfaceProps<FileSurfaceParams>) {
  ...
  return (
    <nav
      aria-label={tPi("extensions.workspaceFile.filePath")}
          ...
          {hasWorkspace ? (
            ...
            >
              <PopoverTrigger
                ...
                aria-label={tPi("extensions.workspaceFile.browsePath", { name: segment.label })}
                title={tPi("extensions.workspaceFile.browsePath", { name: segment.label })}
                className={cn(
                  ...
                  (rootState.status === "loading" && rootState.nodes.length === 0) ? (
                    ...
                    <ExplorerTree
                      ...
                      onOpenFileError={(_error, node) =>
                        setOpenError(
                          tShell("extensions.shared.fileTree.openError", { name: node.name }),
                        )
                      }
                    />
                  ) : null}
