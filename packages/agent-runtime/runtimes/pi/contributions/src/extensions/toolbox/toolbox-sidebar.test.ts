...
test("sidebar counts include built-in prompts and retain known counts while refreshing", () => {
  ...
  assert.equal(formatToolboxCount("idle", builtins, number), "9");
  assert.equal(formatToolboxCount("loading", builtins, number), "9");
  assert.equal(formatToolboxCount("ready", builtins + 2, number), "11");
  assert.equal(formatToolboxCount("loading", builtins + 2, number), "11");
  assert.equal(formatToolboxCount("ready", builtins + 1, number), "10");
