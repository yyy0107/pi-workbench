...
import { PiServerError } from "../../src/core/errors";
import {
  createPiRuntimeHttpRouter,
  type PiRuntimeHttpRouterDependencies,
} from "../../src/transport/runtime-http-router";
import { RPC_REQUEST_BODY_LIMITS } from "../../src/transport/rpc-transport";
...
function harness(overrides: Partial<PiRuntimeHttpRouterDependencies> = {}) {
  const calls: Array<{ name: string; args: unknown[] }> = [];
  const call = (name: string, ...args: unknown[]): void => {
    calls.push({ name, args });
  };
  const dependencies: PiRuntimeHttpRouterDependencies = {
  ...
  return { calls, router: createPiRuntimeHttpRouter(dependencies) };
}
