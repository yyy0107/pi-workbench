...
import {
  WORKSPACE_GIT_LOG_COMMIT_LIMIT,
  type WorkspaceView,
} from "@workbench/agent-runtime-pi-protocol/rpc";

import { PiResourceMutationBusyError } from "../../src/resources/pi-resource-mutation-coordinator";
import {
...
} from "../../src/workspaces/workspace-git";
...
function harness(overrides: Partial<WorkspaceGitServiceDependencies> = {}) {
  ...
  const service = createWorkspaceGitService({
    ...
    resolveWorkspaceStore: () => ({ list: async () => ({ items: [workspace] }) }),
    mutation: {
      async mutate(_cwd, operation) {
        const mutation = await operation();
        reloads.push(mutation.reload);
...
test("rejects missing, invalid, existing, and busy branch mutations with stable errors", async () => {
  ...
  const busy = harness({
    mutation: {
      async mutate() {
        throw new PiResourceMutationBusyError("session-running");
      },
    },
  });
  await assert.rejects(
...
test("rejects unknown workspace ids before running Git", async () => {
  let commands = 0;
  const { service } = harness({
    resolveWorkspaceStore: () => ({ list: async () => ({ items: [] }) }),
    runGit: async () => {
      commands += 1;
      return result();
    },
