...
Mouse input uses viewport-relative CSS coordinates, which can differ from the PNG's pixel dimensions. Scale positions from the displayed screenshot into the reported viewport. `browser_screenshot` supports `format`, JPEG `quality`, and `maxDim`; native CDP scaling bounds the bitmap without changing the page viewport. A full-page capture also contains offscreen document content; scroll to the target and capture the viewport before clicking coordinates there.
