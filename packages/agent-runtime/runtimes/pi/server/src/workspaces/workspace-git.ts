...
import { WORKSPACE_GIT_LOG_COMMIT_LIMIT } from "@workbench/agent-runtime-pi-protocol/rpc";
...
export interface WorkspaceGitStore {
  list(): Promise<{ items: WorkspaceView[] }>;
}

export interface WorkspaceGitMutationPort {
  mutate<Value>(
    cwd: string,
...
export interface WorkspaceGitServiceDependencies {
  ...
  readonly mutation: WorkspaceGitMutationPort;
  readonly resolveWorkspaceStore: () => WorkspaceGitStore;
...
function defaultDependencies(): WorkspaceGitServiceDependencies {
  return {
    canonicalizePath: realpath,
    mutation: {
      mutate(cwd, operation) {
        return getPiResourceMutationCoordinator().mutate({ scope: "project", cwd }, operation);
      },
    },
...
class DefaultWorkspaceGitService implements WorkspaceGitProtocol {
  ...
  async switchBranch(
    ...
    const workspace = await this.requireWorkspace(input.workspaceId);
    try {
      return await this.dependencies.mutation.mutate(workspace.path, async () => {
        ...
        if (status.branch === input.branch) return { value: status, reload: false };
        ...
        return { value: await this.requireRepository(workspace, signal), reload: true };
      });
    } catch (error) {
  ...
  private async requireWorkspace(workspaceId: string): Promise<WorkspaceView> {
    const { items } = await this.dependencies.resolveWorkspaceStore().list();
...
export function createWorkspaceGitService(
  overrides: Partial<WorkspaceGitServiceDependencies> = {},
): WorkspaceGitProtocol {
  return new DefaultWorkspaceGitService({ ...defaultDependencies(), ...overrides });
}
