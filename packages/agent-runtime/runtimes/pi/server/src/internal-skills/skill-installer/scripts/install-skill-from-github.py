#!/usr/bin/env python3
"""Install selected GitHub skill directories into the Pi Runtime's skills directory."""
...
from github_utils import default_skills_dir, github_request
...
def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "pi-skill-installer")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "pi-skill-install")
...
def _download_repo_zip(owner: str, repo: str, ref: str, dest_dir: str) -> str:
    zip_url = f"https://codeload.github.com/{owner}/{repo}/zip/{urllib.parse.quote(ref, safe='')}"
...
def _git_sparse_checkout(
    ...
    # Retry only inside our temporary checkout, never in a user's repository.
...
def main(argv: list[str]) -> int:
    ...
    try:
        ...
        dest_root = os.path.realpath(os.path.expanduser(args.dest or _default_dest()))
        if ".builtin" in dest_root.replace("\\", "/").split("/"):
            raise InstallError("Built-in resources are application-owned; choose a user or project skills directory.")
        ...
        return 0
    except (InstallError, OSError, ValueError, KeyError, zipfile.BadZipFile, subprocess.TimeoutExpired) as exc:
