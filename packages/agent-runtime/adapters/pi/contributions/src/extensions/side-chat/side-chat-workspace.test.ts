...
import { createPiI18n, definePiMessage } from "../../i18n";
...
test("renders the sequence after the localized side-chat title", () => {
  ...
  assert.equal(createPiI18n("en-US").text(title), "Temporary chat (2)");
  assert.equal(createPiI18n("zh-CN").text(title), "临时侧聊 (2)");
});

test("keeps legacy unnumbered side-chat title descriptors resolvable", () => {
  const title = definePiMessage("extensions.sideChat.title");

  assert.equal(createPiI18n("en-US").text(title), "Temporary chat");
  assert.equal(createPiI18n("zh-CN").text(title), "临时侧聊");
});
