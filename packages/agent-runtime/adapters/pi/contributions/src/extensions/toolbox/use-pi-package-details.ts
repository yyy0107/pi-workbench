...
import type { PiPackageCatalogDetailsView } from "@workbench/agent-runtime-pi-protocol/rpc";
...
export function usePiPackageDetails(name: string, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    void resourceClient.describePackageCatalog({ name }).then(
      (value) => {
  ...
  }, [enabled, name, resourceClient, revision]);
