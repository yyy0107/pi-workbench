...
import type { ExtensionView } from "@workbench/agent-runtime-pi-protocol/rpc";
...
export function PiExtensionsSettingsItem({ sectionId, itemId }: SettingsItemComponentProps) {
  ...
  const loadPiExtensionsCatalog = useMemo(
    () =>
      (sessionId: string): Promise<PiExtensionsCatalog> =>
        resourceClient.listExtensions({ sessionId }),
    [resourceClient],
  );
  const {
    sessionId,
