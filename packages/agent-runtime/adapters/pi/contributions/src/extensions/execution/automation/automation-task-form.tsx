...
import { useMainViewService, useNavigationService } from "@workbench/extension-host";
...
export function AutomationTaskForm({ params }: { params: AutomationTaskParams }) {
  ...
  const { automation: automationClient } = usePiExecutionClient();
  ...
  useEffect(() => {
    ...
    void configurationClient.listModelCatalog().then(
      (catalog) => {
        if (disposed) return;
