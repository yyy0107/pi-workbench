...
import { workflowProjectTrustDialogCopy } from "../project-trust-dialog-copy";
...
function CreateWorkflowPage({
  ...
  const titleId = `${domScopeId}-workflow-create-title`;
  const detailsTitleId = `${domScopeId}-new-workflow-details-title`;
  const nameId = `${domScopeId}-new-workflow-name`;
  const workspaceTriggerId = `${domScopeId}-new-workflow-workspace`;
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await catalogStore.getState().refresh(workflowClient);
      mainViews.open(
  ...
  return (
    <section aria-labelledby={titleId} className="bg-background h-full min-h-0 overflow-y-auto">
      ...
      >
        ...
        <section aria-labelledby={detailsTitleId} className="mt-8">
          <h2 id={detailsTitleId} className="text-base font-semibold">
          ...
          {scopeType === "project" ? (
            <div className="mt-4 flex flex-col gap-1.5">
              <label htmlFor={workspaceTriggerId} className="text-sm font-medium">
...
async function saveCurrentDraft(
  workflowClient: PiWorkflowClient,
  editorStore: ReturnType<typeof useWorkflowEditorStoreApi>,
  ...
  try {
    ...
    editorStore.getState().finishSave(saved.document, version);
...
function WorkflowEditorPage({ workflowId }: { workflowId: string }) {
  const { workflow: workflowClient } = usePiExecutionClient();
  const catalogStore = useWorkflowCatalogStoreApi();
  const editorStore = useWorkflowEditorStoreApi();
  const { t } = usePiI18n();
  const trustDialogCopy = workflowProjectTrustDialogCopy(t);
  const mainViews = useMainViewService();
  const { workspaces } = useWorkspaceSelection();
  ...
  }, [editorStore, rightWorkspace, workflowId, workspaceApplicationId]);
  ...
  useEffect(() => {
    ...
    void workflowClient
      ...
      .then((value) => {
        if (cancelled) return;
        editorStore.getState().load(value);
        setLoadState("ready");
      })
    ...
    return () => {
      cancelled = true;
      const state = editorStore.getState();
      if (state.document?.id === workflowId && state.saveState === "dirty") {
        void saveCurrentDraft(workflowClient, editorStore);
      }
    };
  }, [editorStore, workflowClient, workflowId]);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => void saveCurrentDraft(workflowClient, editorStore), 500);
    return () => clearTimeout(timer);
  }, [document, editorStore, editVersion, saveState, workflowClient, workflowId]);
  ...
  const run = async () => {
    setAction("run");
    setNotice(undefined);
    const saved = await saveCurrentDraft(workflowClient, editorStore);
    if (!saved) {
      setAction(undefined);
      return;
    }
  ...
  const publish = async () => {
    ...
    try {
      ...
      editorStore.getState().load(result);
