...
import { useWorkflowEditorStore, useWorkflowEditorStoreApi } from "../execution-state";
...
export function FlowCanvasCore({
  ...
  const addNode = useCallback(
    (type: AddableNodeType, position?: XYPosition) => {
      const currentDocument = editorStore.getState().document;
  ...
  const updateViewport = useCallback<OnMoveEnd>(
    (_, viewport) => {
      const currentViewport = editorStore.getState().document?.graph.editor.viewport;
      if (
