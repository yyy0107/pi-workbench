...
export function AttachmentUnderstandingSettingsItem({
  ...
  const load = useCallback(() => {
    ...
    void configurationClient.describeAttachmentUnderstandingSettings().then(
      (next) => {
  ...
  const loadModelCatalog = useCallback(() => {
    ...
    void Promise.all([
      configurationClient.listModelProviders(),
      configurationClient.listModelCatalog(),
    ]).then(
      ([directory, catalog]) => {
  ...
  const saveRouting = useCallback(
    async (routing: AttachmentUnderstandingRoutingChoice) => {
      ...
      setSaveError(undefined);
      try {
        const updated = await configurationClient.updateAttachmentUnderstandingSettings({
    ...
    [configurationClient, draft, saving, t, view],
  );
  ...
  }, [configurationClient, draft, saving, t, validationError, view]);
