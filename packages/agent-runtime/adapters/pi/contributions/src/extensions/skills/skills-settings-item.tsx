...
import type { SkillView } from "@workbench/agent-runtime-pi-protocol/rpc";
...
export function SkillsSettingsItem({ sectionId, itemId }: SettingsItemComponentProps) {
  ...
  const loadSkills = useMemo(
    () =>
      async (sessionId: string): Promise<readonly SkillView[]> =>
        (await resourceClient.listSkills({ sessionId })).skills,
    [resourceClient],
  );
  const {
    sessionId,
