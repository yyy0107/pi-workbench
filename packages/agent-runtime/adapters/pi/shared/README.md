...
Pi adapter 的 client/server 共用纯逻辑：Runtime 身份、命令投影、消息 reducer、模型能力与会话展示/
分页规则。
...
本包没有根入口，不拥有网络、文件系统、凭据、Pi coding-agent runtime 对象、React 或 assistant-ui
状态。它只把 Pi protocol 投影为可在 adapter 两端复用的确定性逻辑。
