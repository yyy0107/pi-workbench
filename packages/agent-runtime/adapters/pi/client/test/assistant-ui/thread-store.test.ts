...
import type { PiSessionManager, PiThreadStateSnapshot } from "../../src/runtime/manager";
import {
  createPiAgentThreadStore,
  projectPiAgentThreadSnapshot,
} from "../../src/assistant-ui/thread-store";
...
test("forwards subscriptions and optional thread mutations to the Pi manager", async () => {
  ...
  const manager: ThreadStoreManager = {
    ...
    async forkSessionAt({ sessionId, atSeq, sourceTitle }) {
      calls.push(`fork:${sessionId}:${atSeq}:${sourceTitle}`);
      return { sessionId: "session-forked", title: sourceTitle };
    },
  };
  ...
  assert.deepEqual(calls, [
    ...
    "fork:session-1:7:Projected title",
  ]);
});
