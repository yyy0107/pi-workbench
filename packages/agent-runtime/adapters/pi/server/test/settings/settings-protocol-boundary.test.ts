...
const AGENT_SETTINGS_SERVICE = new URL(
  "../../src/settings/agent-settings-service.ts",
  import.meta.url,
);
...
const IMAGE_SETTINGS_STORE = new URL(
  "../../src/attachment-understanding/settings-store.ts",
  import.meta.url,
);
const AGENT_SETTINGS_ROUTES = new URL(
  "../../src/transport/routes/agent-settings-rpc-routes.ts",
  import.meta.url,
);
const WORKBENCH_SETTINGS_ROUTES = new URL(
  "../../src/transport/routes/workbench-settings-rpc-routes.ts",
  import.meta.url,
);
const IMAGE_SETTINGS_ROUTES = new URL(
  "../../src/transport/routes/image-understanding-settings-rpc-routes.ts",
  import.meta.url,
);
...
test("Settings services implement narrow protocols while retaining persistence ownership", async () => {
  ...
  assert.match(workbench, /WorkbenchSettingsProtocol,/);
  assert.match(workbench, /@workbench\/agent-runtime-contracts\/settings/);
  assert.match(
    workbench,
  ...
  assert.doesNotMatch(workbench, /agent-runtime-pi-protocol/);
...
test("the route composition creates Settings groups without retaining transport details", async () => {
  ...
  assert.match(source, /getService: getWorkbenchSettingsService/);
