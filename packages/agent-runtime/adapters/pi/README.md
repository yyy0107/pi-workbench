...
受认证的 HTTP 与 WebSocket 边界访问它。Pi adapter 负责 session 创建、恢复、执行和持久化，但不拥有
...
Composer 因此不导入 Pi adapter package。服务端 `command.list`、冲突解析和命令执行路径保持 Pi
原生实现。
...
认证。Pi adapter 不监听 Web 端口、不加载 Next，也不监督 Runtime 子进程。
