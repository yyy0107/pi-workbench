...
/**
 * Application-owned prompt feedback boundary consumed by an Agent Runtime implementation.
 *
 * UI ownership, rendering state, and workspace scoping stay outside this contract. A runtime only
 * claims feedback for the active thread aliases and then commits or releases that immutable claim.
 */
...
// This compatibility frame is already persisted in conversation history. Keep it byte-for-byte
// stable even though ownership has moved from the Pi integration into the generic client package.
