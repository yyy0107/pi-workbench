...
const PACKAGE_SOURCE_ROOTS = ["../src/", "../lib/"].map((root) =>
  fileURLToPath(new URL(root, import.meta.url)),
);
const CONCRETE_RUNTIME_IMPORT =
  /(?:from\s+|import\s*\()\s*["'](?:@\/runtime\/pi|@workbench\/pi(?:[-/]|["'])|\.\.\/pi)(?:\/|["'])?/;
const GENERIC_THREAD_PRESENTATION_CONSUMERS = [
  "packages/client/ui-terminal/src/terminal-target.ts",
...
test("the generic Agent Runtime client package does not import a concrete implementation", async () => {
  ...
  for (const file of (await Promise.all(PACKAGE_SOURCE_ROOTS.map(productionSources))).flat()) {
