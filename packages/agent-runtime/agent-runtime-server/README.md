...
本包不得依赖 React、Next.js、具体 UI 框架或具体 Agent Runtime。Pi 等实现应在自己的 adapter package 中
实现这些端口，并在 wire/service 边界完成原生字段与稳定错误码的映射。

Source layout: src owns this capability and its contracts; lib contains consumed internal helpers; tests live at the package root. Example consumer: `src/commands.ts` imports `lib/agent-server-error.ts`. Capability and helper code remains TS/TSX; existing build tooling retains its language.
