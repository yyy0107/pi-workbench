...
import type { PiAssistantMessage, PiEvent } from "@workbench/pi-rpc-contracts/messages";
import type {
  SessionMessageChunkData,
  SessionMessageSnapshotPayload,
  SessionMessageUpdatePayload,
} from "@workbench/pi-rpc-contracts/stream";
import { piAssistantToThreadMessage } from "../src/messages";
import { piHistoryFromSessionEvents } from "../src/session-rpc-projection";

const { SessionMessageAccumulator } = (await import(
  new URL("../src/session-message-accumulator.ts", import.meta.url).href
)) as typeof import("../src/session-message-accumulator");
