# Workbench Pi 集成协议与生命周期

入口导航见 [全部包](../README.md)、[SDK 接入](../pi-sdk/README.md)与 [Runtime 适配](README.md)。本文覆盖跨目录的 Pi 集成，源码链接以当前实际 owner 为准。

Workbench Pi Runtime 将 `@earendil-works/pi-coding-agent` 作为具体 Agent Runtime 嵌入
Workbench。Node 实现运行在独立的 `@workbench/runtime-node` Runtime Host 中；Web/Next.js 进程只通过
受认证的 HTTP 与 WebSocket 边界访问它。Pi Runtime implementation 负责 session 创建、恢复、执行和持久化，但不拥有
通用 Host ingress、Web 应用或桌面进程监督。

本目录实现的是
[DeepSeek Harness HTTP / WebSocket 接口参考](../../docs/deepseekharness-api-design.md)中的当前
Workbench 子集，而不是参考文档全部 59 个接口。线协议的类型来源是：

- [`protocol/src/rpc.ts`](./pi-rpc-contracts/src/rpc.ts)：RPC envelope，以及 Host、Workspace、LLM、Settings、
  Session 的请求和响应类型；
- [`protocol/src/stream.ts`](./pi-rpc-contracts/src/stream.ts)：mux/host WebSocket frame 和 payload 联合；
- [`protocol/src/messages.ts`](./pi-rpc-contracts/src/messages.ts)：Pi client 与 legacy
  `/api/pi/**` 使用的 Pi 消息类型。
...
- [`client/src/integration/capabilities.ts`](./pi-runtime-client/src/integration/capabilities.ts) 将同一个 Pi
...
- `projectPiCapabilityError()` 在能力边界将 `PiApiError` 映射为稳定的 Workbench `code` 和可选
  ...
  [Pi Client 的有限公开入口](./pi-runtime-client/README.md#public-boundary)。
...
flowchart TD
  ...
  HTTP --> CS["Authenticated Runtime Host ingress"]
  WS --> CS
  ...
  AUTOMATION --> AUTOMATION_EXECUTOR["Ordinary session launcher"]
  COMMAND -.->|"implements"| COMMAND_PORT["AgentCommandCatalogPort"]
  EXEC_PORT --> PI_SERVER["Pi server implementation"]
  ...
  IMPORT_SERVICE --> IMPORTERS["Codex / Claude Code / Cursor importers"]
...
状态栏与用量弹窗复用同一总量与百分比：首次响应前或压缩后尚无新的 Provider 用量时，使用包含系统
提示词、工具 Schema 和消息的完整分项估算，不把 Pi 的纯消息估算当作 Provider 总量。运行期间每秒
刷新，计入尚未结束的流式思考、正文和工具调用；消息完成后由 Provider 用量校准，避免重复计入输出。
...
浏览器侧不会把这个 Pi RPC DTO 直接暴露给 Workbench。`@workbench/pi-runtime-client`
...
Composer 因此不导入 Pi implementation package。服务端 `command.list`、冲突解析和命令执行路径保持 Pi
原生实现。
...
WorkspaceStore。来源枚举与扫描/导入 DTO 只在 `protocol/src/rpc.ts` 定义一次，服务端领域类型直接复用
...
占用，应先精确确认监听进程，再通过仓库 `pnpm dev` 启动受管的 Web/Runtime 拓扑。

## Runtime Host 与 Web 边界

[`apps/runtime-node`](../../apps/runtime-node/README.md) 是 Node 应用组合根：
...
[`packages/terminal`](../terminal/README.md) 边界，不混入 Pi 的 downlink-only stream。

Web/Next.js 由 `apps/web` 独立拥有。浏览器模式通过 Web 同源边界访问 Runtime，Electron renderer
则使用桌面 sidecar authority；两条路径都必须在任何 Pi session 或 Terminal PTY 分配前完成 Host
认证。Pi Runtime implementation 不监听 Web 端口、不加载 Next，也不监督 Runtime 子进程。

Web 源码重启由根 [`web-runtime-watch`](../../scripts/web-runtime-watch.mjs) 管理。
[`electron-dev-orchestrator`](../../scripts/electron-dev-orchestrator.mjs) 只管理 Desktop renderer
HMR 与 Electron；Electron 从 `.desktop-build` 自有 Runtime，因此 Runtime 或 package 变更后应重启
`pnpm electron:dev`。Pi Package 安装、移除或 session reload 只更新 Pi 运行时资源，不应触发 Web 或
...
当前目录按职责拆为 `packages/pi-sdk/`、`packages/pi-runtime/`、`packages/pi-ui/`；产品安装在 `packages/product/pi-workbench/`。具体包名、职责、源码与导入示例见[包导航](../README.md)。每包保留真实源码、内部辅助模块与既有测试，跨包只使用显式 exports。

`pi-runtime-server/src/session-composition/registry.ts` 每个模块代际调用一次 `createPiSessionRegistry`。SDK 会话实现归 `../pi-sdk/pi-sdk-sessions`；原进程注册表、交互注册表 Symbol、资源协调器、SDK 会话和关闭钩子的作用域保持，具体默认选择由装配层拥有。
...
```bash
pnpm --filter @workbench/pi-runtime-server test
```

OpenCode Go / DeepSeek V4 Flash 的真实流式诊断可单独运行（会产生一次模型请求，不进入自动测试）：

```bash
pnpm --filter @workbench/pi-runtime-server diagnose:stream --self-test
pnpm --filter @workbench/pi-runtime-server diagnose:stream --session /absolute/path/to/session.jsonl --timeout-seconds 300
pnpm --filter @workbench/pi-runtime-server diagnose:stream --transport raw --session /absolute/path/to/session.jsonl --timeout-seconds 900
```
