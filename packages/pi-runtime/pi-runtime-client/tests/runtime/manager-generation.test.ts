...
import type { PiEvent, PiSessionSummary } from "@workbench/pi-rpc-contracts/messages";
import type {
...
import { parsePiContextTraceData } from "@workbench/pi-conversation-adapter/context-trace";
import type {
...
test("restores the submitted draft alongside typing added while a send is pending", async (t) => {
  ...
  await session.actions.addComposerAttachment?.({
    key: "pending.png",
    name: "pending.png",
    source: "data:image/png;base64,pending",
    mediaType: "image/png",
  });
