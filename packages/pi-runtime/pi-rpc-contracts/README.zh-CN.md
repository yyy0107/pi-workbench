...
公开引用入口：`@workbench/pi-rpc-contracts/attachments`, `@workbench/pi-rpc-contracts/messages`, `@workbench/pi-rpc-contracts/rpc`, `@workbench/pi-rpc-contracts/stream`。跨包只使用显式 exports 与 `workspace:*` 依赖；不跨包引用内部源码。

```bash
pnpm --filter @workbench/pi-rpc-contracts typecheck
pnpm --filter @workbench/pi-rpc-contracts test
```
