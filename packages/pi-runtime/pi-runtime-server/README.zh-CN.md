...
公开引用入口：`@workbench/pi-runtime-server/installation`, `@workbench/pi-runtime-server/http`, `@workbench/pi-runtime-server/websocket`, `@workbench/pi-runtime-server/legacy`。跨包只使用显式 exports 与 `workspace:*` 依赖；不跨包引用内部源码。
