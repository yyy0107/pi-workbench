...
const INSTALLED_PACKAGE_SERVICE = new URL(
  "../../../../pi-sdk/pi-sdk-resources/src/installed-package-service.ts",
  import.meta.url,
);
const PACKAGE_CATALOG_SERVICE = new URL(
  "../../../../pi-sdk/pi-sdk-resources/src/package-catalog-service.ts",
  import.meta.url,
);
const INSTALLED_PACKAGE_ROUTES = new URL(
  "../../src/routes/installed-package-rpc-routes.ts",
  import.meta.url,
);
const PACKAGE_CATALOG_ROUTES = new URL(
  "../../src/routes/package-catalog-rpc-routes.ts",
  import.meta.url,
);
...
test("Installed Package transport depends only on its narrow protocol and validators", async () => {
  ...
  assert.match(source, /from "\.\.\/transport\/package-rpc-validators"/);
  assert.match(source, /from "\.\.\/transport\/resource-rpc-validators"/);
...
test("Package services implement narrow protocols while retaining implementation ownership", async () => {
  ...
  assert.match(installed, /from "\.\/project-trust-service"/);
  assert.match(installed, /from "\.\/pi-resource-mutation-coordinator"/);
