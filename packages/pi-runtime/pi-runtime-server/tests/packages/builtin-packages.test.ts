...
} from "@workbench/pi-workbench-runtime/builtin-packages";
import { InstalledPackageService } from "../../src/resource-composition";

import { readPackageResourceDetails } from "@workbench/pi-sdk-resources/package-resource-details";
import { ScopedResourceContextService } from "../../src/resource-composition";
...
test("the built-in browser loads once as a Pi package across reloads and native resource filters", async (t) => {
  ...
  assert.equal(packageCatalog.packages[0]?.name, "@workbench/pi-runtime-browser");
