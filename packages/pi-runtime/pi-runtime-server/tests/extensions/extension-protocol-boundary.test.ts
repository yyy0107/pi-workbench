...
const EXTENSION_SERVICE = new URL(
  "../../../../pi-sdk/pi-sdk-resources/src/extension-service.ts",
  import.meta.url,
);
const EXTENSION_RPC_ROUTES = new URL("../../src/routes/extension-rpc-routes.ts", import.meta.url);
...
test("ExtensionService implements the narrow protocol while retaining Pi resource ownership", async () => {
  ...
  assert.match(source, /getScopedResourceHost/);
  assert.doesNotMatch(source, /sessions\/session-registry/);
  assert.match(source, /from "\.\/pi-resource-mutation-coordinator"/);
  assert.match(source, /from "\.\.\/lib\/internal-extension-path"/);
