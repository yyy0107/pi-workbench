...
[Pi Runtime architecture](../integration.md). They must not import through `@/*` or private `./src/*`
...
Source layout: src owns this capability and its contracts; lib contains consumed internal helpers; tests live at the package root. Example consumer: `src/transport/compaction-rpc-validator.ts` imports `lib/compaction-rpc-validator.ts`. Capability and helper code remains TS/TSX; existing build tooling retains its language.
