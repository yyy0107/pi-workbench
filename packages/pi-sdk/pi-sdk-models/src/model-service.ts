import { createImageInputProbe } from "../lib/image-input-probe";
...
import { createWorkbenchAgentSessionServices } from "./agent-session-services";

import type { PiThinkingLevel } from "@workbench/pi-rpc-contracts/messages";
import {
  imageInputCapability,
  supportedModelThinkingLevels,
  type ModelInputModality,
} from "@workbench/pi-runtime-adapters/models";
...
import type { PiModelRuntimeDependencies } from "@workbench/pi-sdk-ports/models";
...
export interface ModelServiceOptions extends PiModelRuntimeDependencies {
  /** Supplied by the owning host; absent hosts grant no project resource trust. */
...
export async function createModelTestSession(
  services: AgentSessionServices,
  model: Model<Api>,
  dependencies: PiModelRuntimeDependencies = {},
) {
...
export class ModelService implements ModelProviderProtocol, ModelContextWindowProtocol {
  ...
  private readonly resolveProjectTrust: (cwd: string) => Promise<boolean>;
  ...
  constructor(options: ModelServiceOptions = {}) {
    ...
    this.resolveProjectTrust = options.resolveProjectTrust ?? (async () => false);
    this.serviceFactory =
      options.serviceFactory ??
      (async (input) => {
        const services = await createWorkbenchAgentSessionServices(input, options);
        return {
          ...services,
          createTestSession: (model) => createModelTestSession(services, model, options),
        };
      });
  ...
  private async load(): Promise<LoadedModelServices> {
    ...
    const load = async (): Promise<LoadedModelServices> => {
      const services = await this.serviceFactory({
        cwd: this.cwd,
        resourceLoaderReloadOptions: {
          resolveProjectTrust: async () => this.resolveProjectTrust(this.cwd),
        },
      });
      return {
