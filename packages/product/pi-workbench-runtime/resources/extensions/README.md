...
| --------------------------------------------------- | -------------------------------------------------------------------- |
| [browser](browser/index.ts)                         | Register product browser tools with cancellation and session cleanup |
...
For the eight inline extensions, the product's [composition](../../src/extensions.ts) statically imports the factories, injects
...
Browser is the product-owned compatibility package entry. Its factory is bundled from this directory and loaded once through the existing Pi package registration, preserving saved extension/Skill filters. It is not added to the inline list.
