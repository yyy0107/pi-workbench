...
import { truncateHead, defineTool } from "@earendil-works/pi-coding-agent";
...
import type { WorkbenchToolDependencies } from "../tool-runtime/dependencies";
import { validateWorkspace } from "@workbench/pi-sdk-resources/workspace-paths";
...
export function createWorkbenchSettingsTool(
  dependencies: WorkbenchToolDependencies,
  readEnabled: () => Promise<boolean>,
) {
  const piSettings = dependencies.createAgentSettingsService();
  return defineTool({
    ...
    async execute(
      ...
      const settings = dependencies.getHostBindings().workbenchSettings;
      ...
      if (domain === "pi") {
        ...
        if (scope === "project") {
          ...
          const workspace = (await dependencies.listWorkspaces()).find(
        ...
        if (action === "update") {
          ...
          const parsed = dependencies.validateSettingsUpdate({
