...
Workbench defines the `todo` tool and its state in `index.ts`;
[the extension resource](../../resources/extensions/rpiv-todo/index.ts) statically registers it and
restores state from current-branch tool results. The tool name and `details.tasks` / `details.nextId` format remain compatible with rpiv.
