...
Workbench 定时 Composer 提交的 JSON-safe DTO、协议与运行来源 parser。

## Public entry

- `@workbench/automation-contracts`

本包只依赖 `@workbench/core-contracts/model-selection`，不得依赖 React、Next.js、具体 UI 框架、Node 运行时 API、Pi 或其他具体 Agent Runtime。Automation 是定时提示词提交。
