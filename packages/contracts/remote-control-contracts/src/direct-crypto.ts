...
import type { DirectRemoteDirectionV1, DirectSealedEnvelopeV1 } from "./protocol";
...
export type DirectEnvelopeCryptoErrorCode =
...
export class DirectEnvelopeCryptoError extends Error {
  readonly code: DirectEnvelopeCryptoErrorCode;

  constructor(code: DirectEnvelopeCryptoErrorCode) {
    super(`Direct remote envelope rejected: ${code}`);
    this.name = "DirectEnvelopeCryptoError";
    this.code = code;
  }
}

export interface DirectHpkeKeyPair {
...
function assertKeyLength(value: Uint8Array, expected: number): void {
  if (!(value instanceof Uint8Array) || value.byteLength !== expected) {
    throw new DirectEnvelopeCryptoError("invalid_key");
  }
}

function header(envelope: DirectSealedEnvelopeV1): DirectSealedEnvelopeHeaderV1 & {
  readonly keyId: string;
} {
  return {
...
function associatedData(headerValue: object, extra: Uint8Array | undefined): Uint8Array {
  const headerBytes = new TextEncoder().encode(canonicalJson(headerValue));
...
function decodeBase64Url(value: string): Uint8Array {
  if (!/^[A-Za-z0-9_-]+$/u.test(value) || value.length % 4 === 1) {
    throw new DirectEnvelopeCryptoError("invalid_envelope");
  }
  ...
  for (let offset = 0; offset < value.length; offset += 4) {
    ...
    for (let index = 0; index < 4 && offset + index < value.length; index += 1) {
      ...
      if (digit < 0) throw new DirectEnvelopeCryptoError("invalid_envelope");
      bits = (bits << 6) | digit;
      characters += 1;
    }
    bits <<= (4 - characters) * 6;
...
function validateSealInput(input: DirectEnvelopeSealInput): void {
  if (
  ...
  if (
    !parseDirectSealedEnvelopeV1({
      ...
      hpke: { suite: "P256-HKDFSHA256-AES256GCM", enc: "AA", ciphertext: "AA" },
    })
  ) {
    throw new DirectEnvelopeCryptoError("invalid_envelope");
  }
...
function validateOpenInput(input: DirectEnvelopeOpenInput, pairing: boolean): void {
  if (
    !parseDirectSealedEnvelopeV1(input.envelope) ||
    (pairing && input.envelope.contentType !== "pairing") ||
  ...
  if (input.now.getTime() >= Date.parse(input.envelope.expiresAt)) {
    throw new DirectEnvelopeCryptoError("expired");
  }
}

export async function generateDirectHpkeKeyPair(): Promise<DirectHpkeKeyPair> {
  try {
...
export async function sealDirectRemoteEnvelope(
  ...
  try {
    ...
    const sealed = await suite.seal(
      { recipientPublicKey, senderKey: senderPrivateKey, info: DIRECT_REMOTE_HPKE_INFO },
      input.plaintext,
      associatedData(headerWithKey, input.associatedData),
    );
    ...
    if (error instanceof DirectEnvelopeCryptoError) throw error;
    throw new DirectEnvelopeCryptoError("authentication_failed");
  }
}
...
export async function openDirectRemoteEnvelope(
  ...
  validateOpenInput(input, false);
  const recipientPrivateKeyBytes = await input.resolveRecipientPrivateKey(input.envelope.keyId);
  if (recipientPrivateKeyBytes === undefined) throw new DirectEnvelopeCryptoError("key_not_found");
  ...
  try {
    ...
    const plaintext = await suite.open(
      ...
      associatedData(header(input.envelope), input.associatedData),
    );
...
export async function sealDirectPairingEnvelope(
  ...
  validateSealInput({ ...input, senderPrivateKey: new Uint8Array(PRIVATE_KEY_BYTES) });
...
export async function openDirectPairingEnvelope(
  ...
  validateOpenInput({ ...input, senderPublicKey: new Uint8Array(PUBLIC_KEY_BYTES) }, true);
  const recipientPrivateKeyBytes = await input.resolveRecipientPrivateKey(input.envelope.keyId);
  if (recipientPrivateKeyBytes === undefined) throw new DirectEnvelopeCryptoError("key_not_found");
  assertKeyLength(recipientPrivateKeyBytes, PRIVATE_KEY_BYTES);
  try {
