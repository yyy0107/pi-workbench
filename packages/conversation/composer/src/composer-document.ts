...
import {
  LEGACY_COMMAND_DIRECTIVE_TYPE,
  LEGACY_COMPOSER_DIRECTIVE_RE,
  LEGACY_PROJECT_SKILL_DIRECTIVE_TYPE,
  LEGACY_USER_SKILL_DIRECTIVE_TYPE,
} from "../lib/legacy-pi-compat";
import type {
...
function persistedSkillScopeFromDirectiveType(type: string): PersistedSkillScope | undefined {
  if (type === AGENT_PROJECT_SKILL_DIRECTIVE_TYPE || type === LEGACY_PROJECT_SKILL_DIRECTIVE_TYPE) {
    return "project";
  }
  if (type === AGENT_USER_SKILL_DIRECTIVE_TYPE || type === LEGACY_USER_SKILL_DIRECTIVE_TYPE) {
    return "user";
  }
  return undefined;
}
...
export function isAgentComposerDirectiveType(type: string): boolean {
  return (
    ...
    type === LEGACY_COMMAND_DIRECTIVE_TYPE ||
    type === LEGACY_PROJECT_SKILL_DIRECTIVE_TYPE ||
    type === LEGACY_USER_SKILL_DIRECTIVE_TYPE
  );
}
...
function parsedDirectiveMatches(text: string): readonly ParsedDirectiveMatch[] {
  ...
  for (const match of text.matchAll(LEGACY_COMPOSER_DIRECTIVE_RE)) {
  ...
  serialize(item: ComposerTriggerItem): string {
    ...
    const source =
      item.type === WORKBENCH_COMMAND_DIRECTIVE_TYPE
        ? "workbench"
        : item.type === AGENT_COMMAND_DIRECTIVE_TYPE || item.type === LEGACY_COMMAND_DIRECTIVE_TYPE
          ? "agent"
          : undefined;
    if (!source) {
