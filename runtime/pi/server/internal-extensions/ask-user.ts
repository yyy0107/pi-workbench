...
function createWorkbenchAskUserCapabilitySettings(): AskUserCapabilitySettings {
  const service = new WorkbenchSettingsService();
  return {
    async readEnabled() {
      return (await service.describe()).preferences.askUserEnabled !== false;
    },
    subscribe(listener) {
      return subscribeWorkbenchSettingsPreferences(service.stateFile, (preferences) => {
...
export function createAskUserExtension(
  settings: AskUserCapabilitySettings = createWorkbenchAskUserCapabilitySettings(),
): ExtensionFactory {
  return (pi) => {
