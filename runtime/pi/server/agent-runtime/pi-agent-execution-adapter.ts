...
export function createPiAgentExecutionAdapter(
  ...
  return {
    ...
    async resume({ threadId, checkpointId, expectedLeafId }) {
      try {
        await implementation.resumeSession(threadId, checkpointId, expectedLeafId);
      } catch (error) {
        translatePiExecutionError(error);
      }
    },

    async selectBranch({ threadId, leafId }) {
      try {
        await implementation.selectSessionBranch(threadId, leafId);
      } catch (error) {
        translatePiExecutionError(error);
      }
    },

    async updateQueue({ threadId, itemId, mutation }) {
      try {
