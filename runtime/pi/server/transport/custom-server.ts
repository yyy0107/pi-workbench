...
export function isStreamHttpRequest(request: Pick<IncomingMessage, "method" | "url">): boolean {
  if (request.method !== "GET" && request.method !== "HEAD") return false;
  const pathname = requestPathname(request.url);
  return pathname !== undefined && STREAM_PATHNAMES.has(pathname);
}
...
export function createWorkbenchHttpServer(options: WorkbenchHttpServerOptions): Server {
  ...
  const server = createServer((request, response) => {
    ...
    if (isStreamHttpRequest(request)) {
