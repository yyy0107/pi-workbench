...
const moduleHooks = registerHooks({
  resolve(specifier, context, nextResolve) {
    return nextResolve(
      specifier === "./native-workspace-picker" ? `${specifier}.ts` : specifier,
      context,
    );
  },
});
const {
  ...
  new URL("./host-directories.ts", import.meta.url).href
)) as typeof import("./host-directories");
moduleHooks.deregister();

type HostDirectoryErrorCode = import("./host-directories").HostDirectoryErrorCode;
type HostPathRunner = import("./host-directories").HostPathRunner;
