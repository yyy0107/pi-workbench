...
} from "../agent-command-catalog-port";
import {
  AgentExecutionError,
...
} from "../agent-thread-store-port";

const COMMAND_PORT_METHODS = ["getCatalog"] as const;
const EXECUTION_PORT_METHODS = [
  "submit",
  "regenerate",
  "resume",
  "selectBranch",
...
const THREAD_STORE_PORT_METHODS = [
  "list",
  "listSearchDocuments",
  "create",
  "rename",
...
export const WORKBENCH_AGENT_SERVER_CONTRACT_INPUTS = Object.freeze({
  ...
  fork: { threadId: "contract-thread", atEventRevision: 7 },
});
...
export interface WorkbenchAgentServerContractExpectations {
  ...
  readonly searchDocuments: readonly AgentThreadSearchDocument[];
  readonly created: AgentThreadCreateResult;
  readonly renamed: AgentThreadRenameResult;
  readonly forked: Readonly<{ threadId: string }>;
}
...
>({ name, createHarness, expected, failures }: WorkbenchAgentServerContractOptions<Harness>): void {
  describe(`${name} WorkbenchAgentServerAdapter contract`, () => {
    test("exposes one stable identity and the complete narrow port surface", async () => {
    ...
    test("accepts the neutral execution model across prompt, lifecycle, branch, and queue operations", async () => {
      const harness = createHarness();
      try {
        ...
        await execution.resume({
          ...
          expectedLeafId: "contract-leaf",
        });
        await execution.selectBranch({
          threadId: "contract-thread",
          leafId: "contract-branch",
        });
    ...
    test("projects the neutral thread catalog, search, and lifecycle model", async () => {
      const harness = createHarness();
      try {
        ...
        assert.deepEqual(await threads.listSearchDocuments(), expected.searchDocuments);
        assert.deepEqual(
        ...
        assert.deepEqual(
          await threads.fork(WORKBENCH_AGENT_SERVER_CONTRACT_INPUTS.fork),
          expected.forked,
        );
