...
const moduleHooks = registerHooks({
  resolve(specifier, context, nextResolve) {
    if (specifier === "node:fs/promises" && context.parentURL?.endsWith("/file-persistence.ts")) {
      return nextResolve("./file-persistence.test-fs.ts", context);
    }
    return nextResolve(specifier === "./file-persistence" ? `${specifier}.ts` : specifier, context);
  },
});
const { atomicReplaceFile, CrossProcessFileLockTimeoutError, withCrossProcessFileLock } =
  (await import(
    new URL("./file-persistence.ts", import.meta.url).href
  )) as typeof import("./file-persistence");
