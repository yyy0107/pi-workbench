...
const HOST_ROUTES = new URL(
  "../packages/server/local-host-server/src/host-rpc-routes.ts",
  import.meta.url,
);
const LOCAL_APP_ROUTES = new URL(
  "../packages/server/local-host-server/src/local-app-rpc-routes.ts",
  import.meta.url,
);
...
test("the final domain transports depend only on narrow protocols", async () => {
  ...
  assert.match(host, /import type \{ LocalHostProtocol as HostProtocol \}/);
...
test("route composition owns the injectable domain graph while the Router remains thin", async () => {
  ...
  assert.match(installed, /createLocalAppRpcRoutes/);
  assert.match(installed, /createLocalHostRpcRoutes/);
...
test("the Runtime HTTP router owns carriers but never constructs a second service graph", async () => {
  ...
  const ingress = await readFile(
    new URL("../apps/runtime-node/src/composition/runtime-http-router.ts", import.meta.url),
    "utf8",
  );
  assert.ok(ingress.includes("/api/workspace.files.content"));
  assert.match(ingress, /handleRpcPost\(request, method\)/);
  assert.doesNotMatch(runtimeRouterSource, /workspace.files.content|const RPC_ROUTE/);
