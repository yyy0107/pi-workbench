...
The Workbench settings implementation at `packages/product/pi-workbench-runtime/src/workbench-settings/index.ts` demonstrates field selection and JSON offload for both describe and update. It reuses Pi's public budget checks without modifying the SDK or installing a parallel result middleware.
