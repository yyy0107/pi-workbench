...
- It is acceptable for shared/client code to import canonical types or browser-safe pure utilities such as `parseStreamingJson` when the current bundle supports them, as `packages/pi/shared/src/messages.ts` does.
...
- Preserve unrelated worktree changes in `packages/pi/model-server/src/`, protocol stream contracts, and shared message reducers.
