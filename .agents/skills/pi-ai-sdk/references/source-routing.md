...
| Product model/provider/auth implementation | `packages/pi-sdk/pi-sdk-models/src/model-service.ts`                             |
| Coding-agent model runtime construction    | `packages/pi-sdk/pi-sdk-sessions/src/session-registry.ts`                        |
| Browser-facing model RPC types             | `packages/pi-runtime/pi-rpc-contracts/src/rpc.ts`                                |
| Compact streamed-message wire types        | `packages/pi-runtime/pi-rpc-contracts/src/stream.ts`                             |
| Immutable message delta reducer            | `packages/pi-runtime/pi-runtime-adapters/src/messages.ts`                        |
| Server stream compaction/snapshots         | `packages/pi-sdk/pi-sdk-sessions/src/session-registry.ts`                        |
| Client stream accumulation                 | `packages/pi-runtime/pi-conversation-adapter/src/session-message-accumulator.ts` |
| Inline image admission                     | `packages/pi-sdk/pi-sdk-sessions/src/inline-image-admission.ts`                  |

Search before adding an abstraction:

```bash
rg -n '@earendil-works/pi-ai|PiMessagesEvent|parseStreamingJson|builtinProviders' packages/pi-sdk packages/pi-runtime apps/web/next.config.ts
