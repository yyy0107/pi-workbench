...
const TRACE_LOAD_FAILED = defineMessage("extensions.contextTrace.loadFailed");
const TRACE_PERMISSION_REQUIRED = defineMessage("extensions.contextTrace.permissionDescription");
const TRACE_PERSISTENCE_UNAVAILABLE = defineMessage(
...
function viewLabel(t: ReturnType<typeof useI18n>["t"], view: TraceViewMode): string {
  switch (view) {
    case "duration":
...
function detailViewLabel(t: ReturnType<typeof useI18n>["t"], view: ContextTraceDetailView): string {
  switch (view) {
    case "summary":
...
export function ContextTraceSurface({
  ...
  const loadDetail = useCallback(
    (traceId: string) => {
      ...
      void readPiRpcSessionContextTrace({
