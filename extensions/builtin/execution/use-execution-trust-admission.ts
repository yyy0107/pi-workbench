...
export function useExecutionTrustAdmission(onError: (error: unknown) => void) {
  ...
  const admit = useCallback(
    async (
      ...
      setSaveError(false);
      const trust = await describePiProjectTrust({ path: targetPath });
  ...
  const confirm = useCallback(async () => {
    ...
    setSaveError(false);
    try {
      await updatePiProjectTrust({ path: pending.path, trusted: true });
    } catch {
      setSaveError(true);
  ...
  }, [onError, pending, savingDecision]);
  ...
  const decline = useCallback(async () => {
    ...
    setSaveError(false);
    try {
      await updatePiProjectTrust({ path: pending.path, trusted: false });
      setPending(undefined);
    } catch {
      setSaveError(true);
    } finally {
