...
import { FileTypeIcon } from "@/components/workspace-file-tree";
...
const gitBranchStatusListeners = new Set<GitBranchStatusListener>();

function publishGitBranchStatus(workspaceId: string, status: WorkspaceGitStatus): void {
  for (const listener of gitBranchStatusListeners) listener(workspaceId, status);
}

function branchActionError(error: unknown, fallback: "switch" | "create"): BranchActionError {
  if (!(error instanceof PiApiError)) return fallback;
  if (error.code === "session-busy") return "session-busy";
  if (error.code === "git-branch-invalid") return "invalid";
  if (error.code === "git-branch-exists") return "exists";
  return fallback;
}
...
function GitBranchSelector({
  ...
  const [status, setStatus] = useState<WorkspaceGitStatus>();
  ...
  const loadStatus = useCallback(
    async (signal?: AbortSignal) => {
      ...
      setLoadError(false);
      try {
        const nextStatus = await describePiWorkspaceGit(
          { workspaceId },
  ...
  useEffect(() => {
    const listener: GitBranchStatusListener = (changedWorkspaceId, nextStatus) => {
      if (changedWorkspaceId !== workspaceId || activeWorkspaceId.current !== changedWorkspaceId) {
        return;
      }
      requestRevision.current += 1;
      setStatus(nextStatus);
    ...
    gitBranchStatusListeners.add(listener);
    return () => {
      gitBranchStatusListeners.delete(listener);
    };
  }, [workspaceId]);
  ...
  const switchBranch = async (branch: string) => {
    if (!workspaceId || switchingBranch || branch === repository?.branch) return;
    setSwitchingBranch(branch);
    setSwitchError(undefined);
    try {
      const nextStatus = await switchPiWorkspaceGitBranch({ workspaceId, branch });
      if (activeWorkspaceId.current === workspaceId) {
        setStatus(nextStatus);
        publishGitBranchStatus(workspaceId, nextStatus);
        setPendingBranch(undefined);
        setMenuOpen(false);
        setQuery("");
      }
  ...
  const createBranch = async () => {
    ...
    if (!workspaceId || !branch || creating) return;
    setCreating(true);
    setCreateError(undefined);
    try {
      const nextStatus = await createPiWorkspaceGitBranch({ workspaceId, branch });
      if (activeWorkspaceId.current === workspaceId) {
        setStatus(nextStatus);
        publishGitBranchStatus(workspaceId, nextStatus);
        setCreateOpen(false);
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <label className="grid gap-1.5 text-sm font-medium" htmlFor="git-branch-name">
