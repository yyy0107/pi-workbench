...
export const extensionsZhCN = {
  ...
  modelConfig: {
    ...
    adapterCatalogDescription: ({ count }: { count: number }) =>
      `适配器提供了 ${count} 个默认模型。上下文容量覆盖只保存在本地；仅在需要替换默认目录时再自定义模型。`,
    adapterCatalogEmpty: "适配器当前没有提供模型。可以刷新提供方目录，或创建自定义模型目录。",
    ...
    modelName: "显示名称",
    contextWindow: "上下文窗口",
    contextWindowProviderDefault: "Provider 默认值",
    contextWindowLocalOverride: "本地覆盖",
    restoreProviderDefault: "恢复 Provider 默认值",
    editContextWindow: ({ name }: { name: string }) => `编辑${name || "模型"}的上下文容量`,
    ...
    errors: {
      ...
      invalidContextWindow: "请输入 1 到 10,000,000 之间的整数。",
      contextWindowSaveFailed: "无法保存上下文容量覆盖，请重试。",
      contextWindowResetFailed: "无法恢复 Provider 默认值，请重试。",
  ...
  workflows: {
    ...
    create: {
      title: "新建流程",
      description: "选择编辑体验。三种类型共用同一个流程引擎。",
      name: "名称",
      ...
      submit: "创建流程",
      creating: "正在创建…",
      workflowDescription: "通过可视画布构建支持条件与并行分支的 DAG。",
      sopDescription: "创建包含人工审批的可执行线性检查清单。",
      automationDescription: "构建流程并绑定定时或内部事件触发器。",
    },
    actions: {
      publish: "发布",
      publishing: "正在发布…",
